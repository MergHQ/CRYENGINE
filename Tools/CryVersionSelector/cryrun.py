...
def get_solution_path (args):
    basename = os.path.basename (args.project_file)
    file_name = os.path.splitext (basename)[0]
    return os.path.join ('Solutions', "%s.%s" % (file_name, args.platform), file_name + ".sln")
...
def cmd_projgen(args):
    ...
    if cmakelists_dir is not None and os.path.exists(cmakelists_path) and not args.buildmachine:
        ...
        solution_dir = os.path.join (project_path, get_solution_dir (args))
        ...
        print_subprocess (subcmd)
        errcode = subprocess.call(subcmd, cwd = solution_dir)
        if errcode != 0:
            sys.exit (errcode)
        else:
            solution_path = os.path.join (project_path, get_solution_path (args))
            crysolutiongenerator.create_shortcut(solution_path, args.project_file)
