...
{
	if (gEnv->pAISystem)
	  gEnv->pAISystem->Legacy()->RegisterDamageRegion(this, Sphere(ZERO, -1.0f)); // disable
}
...
void CVehicleDamageBehaviorBurn::Activate(bool activate)
{
  if (activate && !m_isActive)
  {
    ...
    m_timerId = m_pVehicle->SetTimer(-1, 20000, this); // total burn of 60 secs
		...
		if( !m_pVehicle->IsDestroyed() && !m_pVehicle->IsFlipped() && pAISystem && pAISystem->IsEnabled() && pAISystem->Legacy()->GetSmartObjectManager() )
		{
			gEnv->pAISystem->Legacy()->GetSmartObjectManager()->SetSmartObjectState(m_pVehicle->GetEntity(), "Exploding");
		}
	...
	if (m_timeCounter <= 0.0f)
	{
      ...
      gEnv->pEntitySystem->QueryProximity(query);
          ...
          float damage = (pEntity->GetId() == m_pVehicle->GetEntityId()) ? m_selfDamage : m_damage;
			...
			if (gEnv->pAISystem)
	      gEnv->pAISystem->Legacy()->RegisterDamageRegion(this, Sphere(worldPos, m_radius));
		}
