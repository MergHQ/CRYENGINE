...
{
	CRY_ASSERT(s_telemetryCollector==NULL);
...
CTelemetryCollector::~CTelemetryCollector()
{
	...
	CRY_ASSERT(s_telemetryCollector==this);
			...
			if (largeSubmitData)
			{
				...
				switch (largeSubmitData->m_state)
				{
					...
					case CTelemetryCollector::SLargeFileSubmitData::k_state_chunk_submitted_send_next_one:
						...
						CRY_ASSERT(largeSubmitData->m_flags&k_tf_chunked);
						break;
	...
	TTelemetrySubmitFlags inFlags)
{
	...
	if (pInProducer)
	{
		...
		if (pLargeSubmitData)
		{
			...
			CRY_ASSERT(inFlags&k_tf_chunked);		// Telemetry producers must be chunked
		...
		case k_writeChunks:
			{
				...
				if (CRY_VERIFY((k_reserveSpace+inMinRequired)<=inBufferSize))
				{
						...
						case eTS_EndOfStream:
							{
								...
								CRY_ASSERT(written<=(inBufferSize-k_reserveSpace));		// something would have gone very wrong if this fires
		...
		case k_writeFooter:
			{
				...
				CRY_ASSERT(footerLength <= static_cast<size_t>(inBufferSize));			// could fix this by splitting footer over multiple produce() calls, but it is an unnecessary complication for the buffer sizes in use
