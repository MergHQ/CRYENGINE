...
{
  CRY_ASSERT(!_isnan(pos.len2()));
  CRY_ASSERT(!_isnan(normal.len2()));
  CRY_ASSERT(pos.len()<25000.f);
...
//	lAnimPos = pCharacter->GetISkeleton()->GetAbsJointByID(endBoneID).t - pOwner->GetSlotLocalTM (characterSlot, false).GetTranslation ();
  ...
  CRY_ASSERT(!_isnan(lAnimPos.len2()));
  ...
  CRY_ASSERT(!_isnan(finalPos.len2()));
  CRY_ASSERT(!_isnan(goalNormal.len2()));
...
void CActor::OnFall(const HitInfo& hitInfo)
{
	...
	if (pAISystem)
	{
		...
		{	
			...
			if(pAIActor)
			{
				AISignals::IAISignalExtraData *pEData = pAISystem->GetSignalManager()->CreateSignalExtraData();	// no leak - this will be deleted inside SendAnonymousSignal
...
IEntity *CActor::LinkToVehicle(EntityId vehicleId) 
{
	...
	if (m_pAnimatedCharacter)
	{
		...
		//CRY_ASSERT(!IsPlayer() || !GetEntity()->IsHidden());
...
float CActor::GetSpeedMultiplier(SActorParams::ESpeedMultiplierReason reason)
{
	...
	if (CRY_VERIFY(reason >= 0 && reason < SActorParams::eSMR_COUNT))
...
void CActor::SetHealth(float health)
{
	if (health <= 0.0f)
	{
		...
		if (IsClient() == false)
		{
			if (gEnv->pAISystem && GetEntity()->HasAI())
				gEnv->pAISystem->Legacy()->DebugReportDeath(GetEntityAIObject(GetEntity()));
		}
...
void CActor::Kill()
{
	...
	IEntity *pEntity = GetEntity();
	if (IAIObject *pAI = GetEntityAIObject(pEntity))
	{
...
bool CActor::LoadGameParams(SmartScriptTable pEntityTable, SActorGameParams &outGameParams)
{
	...
	if (pEntityTable && pEntityTable->GetValue("gameParams", pGameParams))
	{
		...
		{
			CRY_ASSERT((defStance > STANCE_NULL) && (defStance < STANCE_LAST));
...
void CActor::InitGameParams(const SActorGameParams &gameParams, const bool reloadCharacterSounds)
{
	...
	if(reloadCharacterSounds)
	{
		...
		{
			CRY_ASSERT(propertiesTable.GetPtr() != NULL);
...
bool CActor::IsInMercyTime() const
{
	...
	else
	{
		...
		const IAIObject* pAiObject = GetEntityAIObject(GetEntity());
		if (pAiObject != NULL)
		{
...
bool CActor::IsFriendlyEntity(EntityId entityId, bool bUsingAIIgnorePlayer/* = true*/) const
{
	...
	if (!gEnv->bMultiplayer)
	{
		if (GetEntityAIObject(pEntity))
		{
			return (!GetEntityAIObject(pEntity)->IsHostile(GetEntityAIObject(GetEntity()), bUsingAIIgnorePlayer));
		}
	}
	else
	{
	...
	if ((pActorStats != NULL) && (pActorStats->isGrabbed != grabbed))
	{
		if (grabbed)
		{
			...
			if (pAIObject && gEnv->pAISystem)
			{
				...
				if(pAIActor)
				{
					AISignals::IAISignalExtraData *pSData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
