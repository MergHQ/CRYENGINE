...
void CPlayerMindGameBeamEffectLinkedEffect::BetweenPoints(const Vec3& startPos, const Vec3& endPos)
{
	...
	if (m_LightningID == -1)
	{
		CRY_ASSERT(m_EffectConfig != NULL);
		...
		CRY_ASSERT(m_EffectConfig->m_MinRestartDelay <= m_EffectConfig->m_MaxRestartDelay);
...
{
	CRY_ASSERT(targetEntityEffectName != NULL);
	CRY_ASSERT(linkedEffectConfig != NULL);
	CRY_ASSERT(leftBossConnectAttachmentName != NULL);
	CRY_ASSERT(rightBossConnectAttachmentName != NULL);
...
{
	CRY_ASSERT(playerLeftHandPos != NULL);
	CRY_ASSERT(playerRightHandPos != NULL);
...
{
	CRY_ASSERT(leftStartPosOnPlayer != NULL);
	CRY_ASSERT(rightStartPosOnPlayer != NULL);
...
{
	CRY_ASSERT(targetLeftAttachmentPos != NULL);
	CRY_ASSERT(targetRightAttachmentPos != NULL);
