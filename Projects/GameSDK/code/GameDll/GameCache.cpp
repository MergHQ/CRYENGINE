...
#if !defined(_MSC_VER) || _MSC_VER >= 1921 //workaround for compiler bug
		CRY_VERIFY(m_ActorClassLuaCache.insert(TActorClassLuaCacheMap::value_type(pClass, actorClassLuaCache)).second); // Check that it was inserted and not already present
#else
		const bool success = m_ActorClassLuaCache.insert(TActorClassLuaCacheMap::value_type(pClass, actorClassLuaCache)).second;
		CRY_VERIFY(success, "m_ActorClassLuaCache.insert(TActorClassLuaCacheMap::value_type(pClass, actorClassLuaCache)).second");
#endif
	}
	else
	{
...
{
	if (CRY_VERIFY(pEntityScript) && CRY_VERIFY(pProperties))
	{
		...
		CRY_ASSERT(result.second == true); // Assert that it was inserted and not already present
...
{
	CRY_ASSERT(bodyDamageFileName != NULL);
	CRY_ASSERT(bodyDamagePartsFileName != NULL);
