...
void CPickAndThrowWeapon::ThrowObject()
{
	...
	if (pOwnerPlayer != NULL)
	{
		...
		IAIObject *pPlayerAI = pPlayerEntity ? GetEntityAIObject(pPlayerEntity) : NULL;
		if (pPlayerAI)
		{
...
void CPickAndThrowWeapon::OnThrownObjectSeen(const Agent& agent, EntityId objectId, void *pVPickAndThrowWeapon)
{
	...
	CRY_ASSERT(pPickAndThrowWeapon);
	...
	ITargetTrackManager *pManager = pAISystem ? pAISystem->Legacy()->GetTargetTrackManager() : NULL;
	if (pManager && agent.IsValid() && pPlayerAI)
	{
		...
		pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_SENDER, pSignal);
	}
}
...
{
	CRY_ASSERT( (int)m_weaponsharedparams->pPickAndThrowParams->grabTypesParams.size()>m_grabType );
...
int CPickAndThrowWeapon::DoSimpleMeleeHit( const ray_hit& hitResult, EntityId collidedEntityId )
{
	...
	if (pOwnerActor)
	{
		...
		if (IAIObject* pOwnerAI = GetEntityAIObject(pOwnerEntity))
		{
			...
			IAIObject *pTargetAI = pTarget ? GetEntityAIObject(pTarget) : NULL;
			isFriendlyHit = (pTargetAI) ? !pOwnerAI->IsHostile(pTargetAI, false) : false;

			// Send target stimuli
			IAISystem *pAISystem = gEnv->pAISystem;
			ITargetTrackManager *pTargetTrackManager = pAISystem ? pAISystem->Legacy()->GetTargetTrackManager() : NULL;
...
void CPickAndThrowWeapon::PlayCommunication( const char* pCommName, const char* pChannelName )
{
	...
	{
		if (ICommunicationManager* pCommManager = pAISystem->Legacy()->GetCommunicationManager())
		{
...
CPlayer* CPickAndThrowWeapon::GetNPCGrabbedPlayer()
{
	...
	CRY_ASSERT( CPlayer::GetActorClassType() == pActor->GetActorClass() );
...
{
	CRY_ASSERT( !GetOwnerActor() || GetOwnerActor()->IsPlayer() );  
...
void CPickAndThrowWeapon::Start_Throw( EState throwState )
{
	...
	CRY_ASSERT( throwState==eST_PUSHINGAWAY || throwState==eST_PUSHINGAWAY_POWER || throwState==eST_PUSHINGAWAY_WEAK || 
...
void CPickAndThrowWeapon::DebugDraw()
{
	...
	// simple history debug code
	{
		...
		if (m_state<eST_LAST)
		{
			CRY_ASSERT( m_state==stateNames[m_state].state );
			CRY_ASSERT(m_state > 0 && m_state < eST_LAST);
