...
protected:
	...
	void         PatchFail(const char* pInReason);
...
public:
	...
	void                     SetPatchingEnabled(bool inEnable);
	bool                     IsPatchingEnabled() const { return m_patchingEnabled; }

	void                     AddListener(IDataPatcherListener* pInListener);
	void                     RemoveListener(IDataPatcherListener* pInListener);
...
#if DATA_PATCH_DEBUG
	void LoadPatchFromFile(const char* szFilename);
