...
{
	CRY_ASSERT(!request.HasStance() || (request.GetStance() > STANCE_NULL && request.GetStance()  < STANCE_LAST));
...
void CPlayerMovementController::Update( float frameTime, SActorFrameMovementParams& params )
{
	...
	const bool hasControl = (gEnv->bMultiplayer || !GetEntityAIObject(pEntity) || GetEntityAIObject(pEntity)->IsEnabled());
	...
	if (m_pPlayer->IsAIControlled())
	{
		...
		if (cornerSmoother == 2)
		{
			const IAIObject* pAI = GetEntityAIObject(pEntity);
	...
	else
	{
		...
		{		
			...
			if(pWeaponEntity)
			{
				...
				IAIObject* pAIObject = GetEntityAIObject(m_pPlayer->GetEntity());
