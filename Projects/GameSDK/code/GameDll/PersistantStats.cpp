		...
		case eGM_Extraction:
		{
			...
			if (pRoundsModule)
			{
				...
				if (previousRoundWinnerTeamId == clientTeam)
				{
					...
					if (CRY_VERIFY(previousRoundsTeamScores))
					{
			...
			else
			{
				...
				if(pTargetActor != NULL && pShooterPlayer != NULL)
				{
					if(!gEnv->bMultiplayer)
					{
						...
						IAIObject* pShooterAI = GetEntityAIObject(pShooterPlayer->GetEntity());
						IAIObject* pTargetAI = GetEntityAIObject(pTargetActor->GetEntity());
...
void CPersistantStats::OnSPLevelComplete(const char* levelName)
{
  ...
  if (g_pGameCVars->g_difficultyLevelLowestPlayed != -1)
	...
	//	harder difficulty.
	static_assert((eDifficulty_Easy == 1 && eDifficulty_Delta == 4), "Changing_difficulty_level_values_may_break_achievements");
