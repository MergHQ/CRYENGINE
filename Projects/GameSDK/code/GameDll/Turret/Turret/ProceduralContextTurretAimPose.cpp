...
namespace
{
	...
	{
		CRY_ASSERT( 0 <= velocity );
		CRY_ASSERT( 0 <= elapsedSeconds );
...
float CProceduralContextTurretAimPose::VelocityClampYaw( const float yaw, const float targetYaw, const float threshold, const float velocity, const float elapsedSeconds )
{
	...
	CRY_ASSERT( -gf_PI <= yaw );
	CRY_ASSERT( yaw <= gf_PI );
	CRY_ASSERT( -gf_PI <= targetYaw );
	CRY_ASSERT( targetYaw <= gf_PI );
	CRY_ASSERT( 0 <= velocity );
	CRY_ASSERT( 0 <= elapsedSeconds );
...
{
	CRY_ASSERT( m_pSkeletonPose != NULL );
...
{
	CRY_ASSERT( m_pSkeletonAnim != NULL );
...
{
	CRY_ASSERT( horizontalAimJointName );
	CRY_ASSERT( horizontalAimJointName[ 0 ] != 0 );
	if ( !CRY_VERIFY(m_pSkeletonPose != NULL) )
	{
...
{
	CRY_ASSERT( weaponJointName );
	CRY_ASSERT( weaponJointName[ 0 ] != 0 );
	if ( !CRY_VERIFY(m_pSkeletonPose != NULL) )
	{
...
{
	CRY_ASSERT( aimPosesOriginJointName );
	CRY_ASSERT( aimPosesOriginJointName[ 0 ] != 0 );
	if ( !CRY_VERIFY(m_pSkeletonPose != NULL) )
	{
