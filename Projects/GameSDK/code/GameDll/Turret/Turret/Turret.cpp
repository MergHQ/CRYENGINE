...
CTurret::~CTurret()
{
	...
	if ( pGameObject != NULL )
	{
		...
		CRY_ASSERT( pScriptBindTurret != NULL );
...
void CTurret::InitAiRepresentation( const EInitAiRepresentationMode mode )
{
	...
	gEnv->pAISystem->Legacy()->GetAIObjectManager()->CreateAIObject(params);
...
void CTurret::RemoveAiRepresentation()
{
	...
	gEnv->pAISystem->Legacy()->GetAIObjectManager()->RemoveObjectByEntityId(GetEntityId());
...
void CTurret::NotifyAiThatTurretIsRemoved()
{
	...
	IEntity* const pEntity = GetEntity();
	IAIObject* const pAiObject = GetEntityAIObject(pEntity);
	gEnv->pAISystem->Legacy()->NotifyTargetDead( pAiObject );
}
...
{
	IEntity* const pEntity = GetEntity();
	const IAIObject* const pAiObject = GetEntityAIObject(pEntity);
	if ( pAiObject != NULL )
	{
...
void CTurret::NotifyDestroyed( const bool hasBeenDestroyedByPlayer /* = false */ ) const
{
	...
	if ( IAISystem* pAISystem = gEnv->pAISystem )
	{
		...
		const EntityId entityId = GetEntityAIObject(pTurretEntity) ? GetEntityAIObject(pTurretEntity)->GetEntityID() : INVALID_ENTITYID;
		pAISystem->GetSignalManager()->SendAnonymousSignal(
...
void CTurret::InitActionController()
{
	...
	CRY_ASSERT( pMannequinInterface != NULL );
	...
	CRY_ASSERT( pAnimationDatabaseTurret != NULL );
...
{
	CRY_ASSERT( fragmentName != NULL );
	CRY_ASSERT( fragmentName[ 0 ] != 0 );
...
void CTurret::RemoveItemAttachments()
{
	...
	CRY_ASSERT( pAttachmentManager != NULL );
...
void CTurret::CreateItemAttachments()
{
	...
	CRY_ASSERT( pPrimaryWeaponAttachment != NULL );
...
void CTurret::InitWeapons()
{
	...
	CRY_ASSERT( m_primaryWeaponId == 0 );
...
void CTurret::UpdateWeaponTarget()
{
	...
	CRY_ASSERT( 0.0f < DEFAULT_TURRET_ACCURACY_RANGE );
...
bool CTurret::IsEntityCloaked( IEntity* pTargetEntity ) const
{
	...
	const IAIObject* const pTargetAiObject = GetEntityAIObject(pTargetEntity);
...
bool CTurret::IsEntityHostileAndThreatening( IEntity* pTargetEntity ) const
{
	...
	IAIObject* const pTurretAiObject = GetEntityAIObject(pTurretEntity);
	IAIObject* pTargetAiObject = GetEntityAIObject(pTargetEntity);

	{
		...
		if ( pVehicle != NULL )
		{
			...
			if ( pDriverActor != NULL )
			{
				...
				pTargetAiObject = GetEntityAIObject(pDriverEntity);
			}
		}
	}
...
bool CTurret::IsEntityInVVV( IEntity* pTargetEntity ) const
{
	...
	IEntity* const pTurretEntity = GetEntity();
	const IAIObject* const pTurretAiObject = GetEntityAIObject(pTurretEntity);
	IF_UNLIKELY ( pTurretAiObject == NULL )
	{
		return false;
	}

	const float perceptionScale = gEnv->pAISystem->Legacy()->GetGlobalVisualScale( pTurretAiObject );
...
{
	CRY_ASSERT( pTargetEntity != NULL );
	if ( IAIObject* const pTargetAiObject = GetEntityAIObject(pTargetEntity) )
	{
		AISignals::IAISignalExtraData* const pData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
		...
		gEnv->pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_SENDER, signal);
	}
}
...
void CTurret::SetEyeVisionRangeScale( const float scale, const float timeoutSeconds )
{
	...
	CRY_ASSERT( pCurrentObserverParams );
...
{
	ITargetTrackManager* const pTargetTrackManager = gEnv->pAISystem->Legacy()->GetTargetTrackManager();
	...
	const IEntity* const pTurretEntity = GetEntity();
	const tAIObjectID aiObjectId = GetEntityAIObjectID(pTurretEntity);
...
void CTurret::NotifyGroupTargetSpotted( const IEntity* pTargetEntity )
{
	...
	IAIObject* const pGroupMemeberAiObject = gEnv->pAISystem->Legacy()->GetGroupMember( groupId, 0 );
	...
	ITargetTrackManager* const pTargetTrackManager = gEnv->pAISystem->Legacy()->GetTargetTrackManager();
	CRY_ASSERT( pTargetTrackManager );
	...
	gEnv->pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_GROUPONLY, signal);
}
