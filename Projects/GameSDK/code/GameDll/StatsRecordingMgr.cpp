...
void CStatsRecordingMgr::BeginSession()
{
	if (IsTrackingEnabled())
	{
		if (m_statsStorage->IsLockedForSerialization())
		{
			...
			CRY_ASSERT(m_sessionTracker==NULL);
		}
		else
		{
...
void CStatsRecordingMgr::BeginRound()
{
	if (IsTrackingEnabled() && m_sessionTracker)
	{
		...
		CRY_ASSERT(m_roundTracker);		// tracker will have been assigned and setup by callback
	...
	IActor			*inPlayerActor)
{
	...
	if ( IStatsTracker *tracker = GetStatsTracker(inPlayerActor) )
	{
		...
		if(!gEnv->bMultiplayer)
		{
			IAIObject* pAI = GetEntityAIObject(inPlayerActor->GetEntity());
			if(pAI)
			{
...
{
	CRY_ASSERT(ser.GetSerializationTarget() == eST_SaveGame);
