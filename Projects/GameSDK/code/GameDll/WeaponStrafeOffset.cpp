...
namespace
{
	...
	float SignedPow_2(float linearVal, float curvePower)
	{
...
QuatT CStrafeOffset::Compute(float frameTime)
{
	...
	float interpFront = SignedPow_2(clamp_tpl(velocityDerivative.y * m_staticParams.velocityInterpolationMultiplier, -1.0f, 1.0f), m_staticParams.accelerationSmoothing);
	...
	float interpSide = SignedPow_2(clamp_tpl(velocityDerivative.x * m_staticParams.velocityInterpolationMultiplier, -1.0f, 1.0f), m_staticParams.accelerationSmoothing);
