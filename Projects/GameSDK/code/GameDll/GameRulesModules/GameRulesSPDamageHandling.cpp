		...
		if (IScriptTable* victimScript = pVictimEntity->GetScriptTable())
		{
			...
			if (onHitFunc || onDeadHitFunc)
			{
				...
				else if (!canKill)
				{
					...
					IAIObject* shooterAI = shooter ? GetEntityAIObject(shooter) : 0;
					IAIActor* aiActor = shooterAI ? shooterAI->CastToIAIActor() : 0;

					if (aiActor && shooter != pVictimEntity && !aiActor->CanDamageTarget(GetEntityAIObject(pVictimEntity)))
					{
#ifndef _RELEASE
...
{
	CRY_ASSERT(pOffenderVehicle && pVictimActor);
	...
	else
	{
		...
		{
			// AI hitting AI
			uint8 driverFaction = GetEntityAIObject(pActorDriver->GetEntity())->GetFactionID();
			uint8 victimFaction = GetEntityAIObject(pVictimActor->GetEntity())->GetFactionID();
...
{
	CRY_ASSERT(localClientActor.IsClient());
