...
{
	CRY_ASSERT(acceptableFrameLatency < 128); //If this is too high there is the potential for the framesSinceLastCheck to overflow
	...
	else
	{
		...
		CRY_ASSERT(m_numUsedVisTableEntries <= kMaxVisTableEntries);
	}
...
SDeferredLinetestReceiver * CPlayerVisTable::GetAvailableDeferredLinetestReceiver(SDeferredLinetestBuffer& visBuffer)
{
	...
	CRY_ASSERT(!"Failed to find free processing entry when one was present according to m_numLinetestsCurrentlyProcessing");
