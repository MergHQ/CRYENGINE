...
namespace 
{
	...
	bool IsPointingToFriendly(CPlayer* pPlayer)
	{
		...
		IAIObject* pPlayerAI = GetEntityAIObject(pPlayer->GetEntity());
		IAIObject* pTargetAI = GetEntityAIObject(pTargetEntity);
...
void CItem::Select(bool select)
{
	...
	if (select)
	{
		...
		if (pOwnerActor)
		{
			...
			if (!gEnv->bMultiplayer && pAISystem && pAISystem->Legacy()->GetSmartObjectManager())
			{
				pAISystem->Legacy()->GetSmartObjectManager()->ModifySmartObjectStates( pOwnerEntity, GetEntity()->GetClass()->GetName() );
				pAISystem->Legacy()->GetSmartObjectManager()->ModifySmartObjectStates( pOwnerEntity, "WeaponDrawn" );
			}
		}    
	...
	else
	{
		...
		// update smart objects states
		if ( !gEnv->bMultiplayer && pAISystem && pAISystem->Legacy()->GetSmartObjectManager() && pOwnerActor && !pOwnerActor->IsDead() )
		{
			CryFixedStringT<256> tmpString( "-WeaponDrawn," );
			tmpString += GetEntity()->GetClass()->GetName();
			pAISystem->Legacy()->GetSmartObjectManager()->ModifySmartObjectStates( pOwnerActor->GetEntity(), tmpString.c_str() );
		}
