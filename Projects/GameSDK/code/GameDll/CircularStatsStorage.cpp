...
{
	CRY_ASSERT(m_states==NULL && m_timelines==NULL);
	...
	const CDoubleLinkedElement		*inElement)
{
	...
	CRY_ASSERT((&result->m_timelineLink)==inElement);
	...
	const CDoubleLinkedElement		*inElement)
{
	...
	CRY_ASSERT((&result->link)==inElement);
	...
	int					*pOutWritten)
{
	...
	if (pStat)
	{
		...
		{
			CRY_ASSERT(state.dataWritten>0);
		...
		case eSNS_Dead:
			{
				...
				CRY_ASSERT(m_state==k_notStartedProducing);		// don't mess with array once iteration has started!
			}
			break;
