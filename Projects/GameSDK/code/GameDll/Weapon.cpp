...
void CWeapon::FullSerialize(TSerialize ser)
{
	...
	if (GetOwnerId())
	{
		...
		if (ser.IsReading())
		{
			if (!CRY_VERIFY(numFiremodes == GetNumOfFireModes()))
...
bool CWeapon::IsValidAssistTarget(IEntity* pEntity, IEntity* pSelf, bool includeVehicles /*=false*/)
{
	...
	if (!pActor && includeVehicles && pAI)
	{
		...
		if (pVehicle && pVehicle->GetStatus().health > 0.f && pAI->IsHostile(GetEntityAIObject(pSelf), false))
			return true;
	}
	...
	return (pEntity != pSelf && !pEntity->IsHidden() &&
	        ...
	        pAI->IsHostile(GetEntityAIObject(pSelf), false));
}
...
void CWeapon::MountAt(const Vec3& pos)
{
	...
	gEnv->pAISystem->Legacy()->GetAIObjectManager()->CreateAIObject(AIObjectParams(AIOBJECT_MOUNTEDWEAPON, 0, GetEntityId()));
}
...
const SPlayerMovementModifiers& CWeapon::GetPlayerMovementModifiers() const
{
	...
	{
		CRY_ASSERT((m_zmId >= 0) && (m_zmId < (int)m_weaponsharedparams->zoomedMovementModifiers.size()));
