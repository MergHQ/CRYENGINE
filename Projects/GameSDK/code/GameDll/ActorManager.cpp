...
void CActorManager::ReallocateMemoryForNActors(int iNumActorsRequired)
{
	...
	CRY_ASSERT((((INT_PTR)pMemory) & 127) == 0);	//Double check 128 byte align
...
void CActorManager::Update(float dt)
{
	...
	IAIObject* pPlayerAIObject = pLocalPlayer ? GetEntityAIObject(pLocalPlayer->GetEntity()) : NULL;
	...
	while (pActor != NULL)
	{
		if (pActor != pLocalPlayer)
		{
			...
			const IAIObject* pAIObject = GetEntityAIObject(pEntity);
