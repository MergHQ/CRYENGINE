...
void CWeapon::OnShoot(EntityId shooterId, EntityId ammoId, IEntityClass* pAmmoType, const Vec3 &pos, const Vec3 &dir, const Vec3&vel)
{
	...
	if (pActor)
	{
		...
		{
			if (IAIObject *pAIObject = GetEntityAIObject(pActor->GetEntity()))
			{
				...
				gEnv->pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_LEADER, pSignal);
			}

			gEnv->pAISystem->Legacy()->DisableGlobalPerceptionScaling();
		}
	}
