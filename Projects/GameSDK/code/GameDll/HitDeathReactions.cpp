...
struct CHitDeathReactions::SPredFindValidReaction
{
	...
	bool operator ()(const SReactionParams& reactionParams) const
	{
		...
		if( bFound && gEnv->bMultiplayer && reactionParams.mannequinData.actionType == SReactionParams::SMannequinData::EActionType_Coop )
		{
			...
			if ( CRY_VERIFY(piOptionalAnimationADB != NULL) )
			{
				SFragTagState matchedTagState; 
...
m_bInSmartObject(false)
{
	...
	{
		m_iGoalPipeId = gEnv->pAISystem->Legacy()->AllocGoalPipeId();
	}
}
...
IPipeUser* CHitDeathReactions::GetAIPipeUser() const
{
	...
	if (IsAI())
	{
		IAIObject* pAI = GetEntityAIObject(m_actor.GetEntity());
...
bool CHitDeathReactions::SendAISignal(const AISignals::ISignalDescription& signalDesc, AISignals::IAISignalExtraData *pData, bool bWaitOpOnly) const
{
	...
	IAIObject *pAI = (IsAI() ? GetEntityAIObject(m_actor.GetEntity()) : NULL);
