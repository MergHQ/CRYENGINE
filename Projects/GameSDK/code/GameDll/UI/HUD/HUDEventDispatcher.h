...
struct SHUDEventData
{
	...
	int GetInt() const { CRY_ASSERT(m_type==eSEDT_int); return m_int; }
	float GetFloat() const { CRY_ASSERT(m_type==eSEDT_float); return m_float; }
	bool GetBool() const { CRY_ASSERT(m_type==eSEDT_bool); return m_bool; }
	const void* GetPtr() const { CRY_ASSERT( (m_pointer==NULL) || (m_type==eSEDT_voidptr) ); return m_pointer; }
};
...
public:
	...
	{
#if !defined(DEDICATED_SERVER)
		CRY_ASSERT(m_data->size()<MAX_HUDEVENT_DATA);
...
private:
	...
	static uint8 GetNextFreeDataId()
	{
		...
		CRY_ASSERT(false, "No more HUD event data available");
		return 0;
	}
