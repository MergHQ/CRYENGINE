...
{
	IAIObject* pAIObject = GetEntityAIObject(pTargetEntity);
	
	if(pAIObject != NULL)
	{
		if(pAIObject->IsHostile(GetEntityAIObject(pClientEntity), false))
		{
			...
			int groupCount = pAISystem->GetGroupCount(targetAIGroup, AILegacy::GROUP_ENABLED);
			for (int i = 0; i < n; i++)
			{
				IAIObject*	pMember = pAISystem->GetGroupMember(groupID, i, AILegacy::GROUP_ENABLED);
