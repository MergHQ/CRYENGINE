...
bool CPlayerCamera::Update(SViewParams& viewParams, float frameTime )
{
	...
	CRY_ASSERT((currentCameraMode >= eCameraMode_Default) && (currentCameraMode < eCameraMode_Last));
	...
	{
		CRY_ASSERT(m_previousCameraMode >= eCameraMode_Invalid && m_previousCameraMode < eCameraMode_Last);
		CRY_ASSERT(m_currentCameraMode >= 0 && m_currentCameraMode < eCameraMode_Last);
	...
	CRY_ASSERT(currentCameraMode >= 0);
	...
	{
		CRY_ASSERT(m_transitionCameraMode >= 0);
		QuatT newCamTrans;
		QuatT newTrans(viewParams.rotation, viewParams.position);

    CRY_ASSERT(0 <= m_transitionCameraMode && m_transitionCameraMode < eCameraMode_Last);
