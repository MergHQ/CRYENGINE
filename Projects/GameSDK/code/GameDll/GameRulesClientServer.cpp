	...
	float* resultHealth, bool* resultDead) const
{
	...
	IF_LIKELY(pEntity != NULL)
	{
		...
		else
		{
			...
			{
				CRY_ASSERT(g_pGame != NULL);

				IGameFramework* pGameFramework = g_pGame->GetIGameFramework();
				CRY_ASSERT(pGameFramework != NULL);
...
{
	CRY_ASSERT((int)m_moduleRMIListenersVec.size() >	params.m_listenerIndex);
...
{
	CRY_ASSERT(0 <=	params.m_listenerIndex);
	CRY_ASSERT((int)m_moduleRMIListenersVec.size() >	params.m_listenerIndex);
...
IMPLEMENT_RMI(CGameRules, ClPredictionFailed)
{
	...
	CRY_ASSERT(pEntity && pNetContext && !pNetContext->IsBound(params.predictionHandle));
