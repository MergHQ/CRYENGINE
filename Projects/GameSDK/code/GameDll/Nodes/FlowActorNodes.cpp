...
public:
	...
	{
		IAIObject* pAI = GetEntityAIObject(pActor->GetEntity());
		...
		IAIObject* pClientAI = GetEntityAIObject(gEnv->pGameFramework->GetClientActor()->GetEntity());
...
public:
	...
	void CheckNearestActor(SActivationInfo* pActInfo)
	{
		...
		while (IActor* pActor = actorIt->Next())
		{
			if (pActor)
			{
				const IAIObject* pAIObject = GetEntityAIObject(pActor->GetEntity());
				if (pAIObject)
				{
		...
		if (pNearestActor)
		{
			...
			ActivateOutput(pActInfo, eOUT_Faction, int(GetEntityAIObject(pNearestActor->GetEntity())->GetFactionID()));
