...
void CSpectacularKill::End(bool bKillerDied/* = false*/)
{
	...
	if(pTarget)
	{
		...
		if (!pTarget->IsPlayer() && pTarget->GetEntity()->HasAI())
			GetEntityAIObject(pTarget->GetEntity())->Event(AIEVENT_ENABLE, 0);
	...
	// Enable AI again (for what it's worth - this helps editor)
	if (m_pOwner && m_pOwner->GetEntity()->HasAI())
		GetEntityAIObject(m_pOwner->GetEntity())->Event(AIEVENT_ENABLE, 0);
...
bool CSpectacularKill::StartOnTarget(CActor* pTargetActor)
{
	...
	{
		// Disable AI
		if (!pTargetActor->IsPlayer() && pTargetActor->GetEntity()->HasAI())
			GetEntityAIObject(pTargetActor->GetEntity())->Event(AIEVENT_DISABLE, 0);

		if (!m_pOwner->IsPlayer() && m_pOwner->GetEntity()->HasAI())
			GetEntityAIObject(m_pOwner->GetEntity())->Event(AIEVENT_DISABLE, 0);
