...
#undef CHECKQNAN_FLT
	...
	CRY_ASSERT(((*alias_cast<unsigned*>(&x)) & 0xff000000) != 0xff000000u && (*alias_cast<unsigned*>(&x) != 0x7fc00000))
#else
...
{
static constexpr Cry::Audio::ControlId playerViewSwitchId = Cry::Audio::StringToId("1stOr3rdP");
static constexpr Cry::Audio::ControlId fpViewSwitchStateId = Cry::Audio::StringToId("1stP");
static constexpr Cry::Audio::ControlId tpViewSwitchStateId = Cry::Audio::StringToId("3rdP");
}
...
CPlayer::CPlayer()
	...
	, m_fOxygenLevel(1.0f)
	, m_waterEnter(Cry::Audio::InvalidControlId)
	, m_waterExit(Cry::Audio::InvalidControlId)
	, m_waterDiveIn(Cry::Audio::InvalidControlId)
	, m_waterDiveOut(Cry::Audio::InvalidControlId)
	, m_waterInOutSpeed(Cry::Audio::InvalidControlId)
{
	...
	for (int i = 0; tmpSoundTable[i].soundID != ESound_Player_Last; ++i)
	{
		...
		CRY_ASSERT(soundID >= ESound_Player_First && soundID < ESound_Player_Last);
	...
	m_waterEnter = Cry::Audio::StringToId("water_enter");
	m_waterExit = Cry::Audio::StringToId("water_exit");
	m_waterDiveIn = Cry::Audio::StringToId("water_dive_in");
	m_waterDiveOut = Cry::Audio::StringToId("water_dive_out");
	m_waterInOutSpeed = Cry::Audio::StringToId("water_in_out_speed");
}
	...
	case ENTITY_EVENT_RESET:
		{
			if (gEnv->IsEditor())
			{
				if (event.nParam[0])
				{
					if (IsClient())
					{
						CRY_ASSERT(GetEntityId() == gEnv->pGameFramework->GetClientActor()->GetEntityId());
...
int CPlayer::IsGod()
{
	...
	IAIActor* pAIActor = CastToIAIActorSafe(GetEntityAIObject(GetEntity()));
	if (pAIActor)
	{
...
{
	CRY_ASSERT(m_stats.spectatorInfo.mode == eASM_Follow || m_stats.spectatorInfo.mode == eASM_Killer);
...
{
	CRY_ASSERT(m_stats.spectatorInfo.mode == eASM_Fixed);
...
void CPlayer::NetSerialize_InputClient(TSerialize ser, bool bReading)
{
	...
	CRY_ASSERT(serializedInput.stance == static_cast<uint8>(STANCE_NULL) ||
...
void CPlayer::PostSerialize()
{
	...
	if (IsClient())
	{
		...
		IAIObject* pAIObject = GetEntityAIObject(GetEntity());
		if (pAIObject)
		{
...
void SPlayerStats::Serialize(TSerialize ser, EEntityAspects aspects)
{
	...
	if (CRY_VERIFY(ser.GetSerializationTarget() != eST_Network))
	{
...
void CPlayer::ExecuteFootStep(ICharacterInstance* pCharacter, const float frameTime, const int32 nFootJointID)
{
	...
	if (GetBoneID(boneId) >= 0)
	{
		...
		Cry::Audio::ControlId playerViewSwitchStateId = IsThirdPerson() ? PlayerAudioUtils::tpViewSwitchStateId : PlayerAudioUtils::fpViewSwitchStateId;
		...
		if (feetWaterLevel != WATER_LEVEL_UNKNOWN)
		{
			...
			if (depth >= 0.0f)
			{
				...
				if (!gEnv->bMultiplayer && IsClient() && IsSprinting())
				{
					...
					tAIObjectID targetAIObjectId = GetEntity()->HasAI() ? GetEntityAIObjectID(GetEntity()) : 0;
					gEnv->pAISystem->Legacy()->GetTargetTrackManager()->HandleStimulusEventInRange(targetAIObjectId, "SoundWaterRipple", stimulusEventInfo, radius);
		...
		if (!gEnv->bMultiplayer && m_params.footstepGearEffect)
		{
			...
			{
				if (IAIObject* pAI = GetEntityAIObject(GetEntity()))
				{
...
void CPlayer::ExecuteFootStepsAIStimulus(const float relativeSpeed, const float noiseSupression)
{
	if (IPerceptionManager::GetInstance())
	{
		...
		if (fFootstepSpeed > FLT_EPSILON)
		{
			...
			{
				SAIStimulus stim(AISTIM_SOUND, AILegacy::AISOUND_MOVEMENT, GetEntityId(), 0,
...
void CPlayer::NetKill(const KillParams& killParams)
{
...
#if !defined(_RELEASE)
	CRY_ASSERT(!m_dropCorpseOnDeath, "Getting multiple deaths for %s '%s' (%s%shealth=%8.2f)", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), IsClient() ? "client, " : "", GetEntity()->HasAI() ? "AI, " : "", m_health.GetHealth());
#endif
	...
	if (GetEntity() && GetEntity()->HasAI())
	{
		GetEntityAIObject(GetEntity())->Event(AIEVENT_DISABLE, NULL);
	}
...
void CPlayer::PlaySound(EPlayerSounds soundID, bool play, const char* paramName, float paramValue, const char* paramName2, float paramValue2, float volume, bool playOnRemote)
{
	...
	if (IsClient() || playOnRemote) //currently this is only supposed to be heard by the client (not 3D, not MP)
	{
		...
		case CPlayer::ESound_DiveIn:
			...
			{
				if (m_waterDiveIn != Cry::Audio::InvalidControlId)
				{
					if (m_waterInOutSpeed != Cry::Audio::InvalidControlId)
					{
		...
		case CPlayer::ESound_DiveOut:
			if (m_pIEntityAudioComponent)
			{
				if (m_waterDiveOut != Cry::Audio::InvalidControlId)
				{
					if (m_waterInOutSpeed != Cry::Audio::InvalidControlId)
					{
						m_pIEntityAudioComponent->SetParameter(m_waterInOutSpeed, paramValue);
		...
		case CPlayer::ESound_WaterEnter:
			if (m_pIEntityAudioComponent && m_waterEnter != Cry::Audio::InvalidControlId)
			{
				m_pIEntityAudioComponent->ExecuteTrigger(m_waterEnter);
			}
			break;
		case CPlayer::ESound_WaterExit:
			if (m_pIEntityAudioComponent && m_waterExit != Cry::Audio::InvalidControlId)
			{
...
void CPlayer::AnimationEvent(ICharacterInstance* pCharacter, const AnimEventInstance& event)
{
	...
	if (isMainCharacter)
	{
		...
		else if (event.m_EventNameLowercaseCRC32 == audioTriggerCRC)
		{
			...
			{
				Cry::Audio::AuxObjectId auxObjectId = Cry::Audio::InvalidAuxObjectId;
				...
				if (event.m_BonePathName && event.m_BonePathName[0] && pCharacter)
				{
					...
					if (nJointID >= 0)
					{
						auxObjectId = stl::find_in_map(m_cJointAudioProxies, nJointID, Cry::Audio::InvalidAuxObjectId);
				...
				Cry::Audio::ControlId const triggerId = Cry::Audio::StringToId(event.m_CustomParameter);
		...
		{
			if (IAIActor* aiActor = CastToIAIActorSafe(GetEntityAIObject(GetEntity())))
			{
...
void CPlayer::LeaveAllPlayerPlugins()
{
	for (int i = 0; i < m_numActivePlayerPlugins; i++)
	{
		...
		CRY_ASSERT(m_activePlayerPlugins[i]->IsEntered());
...
{
	CRY_ASSERT(turnThresholdAngle >= 0.0f);
	CRY_ASSERT(turnThresholdTime >= 0.0f);
