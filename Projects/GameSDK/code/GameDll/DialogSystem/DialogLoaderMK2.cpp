...
bool CDialogLoaderMK2::GetActor(const char* actor, int& outID)
{
	...
	if (!CRY_VERIFY(actor != 0 && *actor != '\0')) // safety
	{
		outID = 0;
		return true;
	}
...
void CDialogLoaderMK2::ResetLine(CDialogScript::SScriptLine& scriptLine)
{
	...
	scriptLine.m_audioID = Cry::Audio::InvalidControlId;
	//scriptLine.m_sound = "";
	scriptLine.m_anim = "";
	scriptLine.m_facial = "";
...
bool CDialogLoaderMK2::ReadLine(const XmlNodeRef& lineNode, CDialogScript::SScriptLine& line, const char* scriptID, int lineNumber)
{
	...
	{
		line.m_audioID = Cry::Audio::StringToId(szTriggerName);
	}
	else
	{
		line.m_audioID = Cry::Audio::InvalidControlId;
	}
