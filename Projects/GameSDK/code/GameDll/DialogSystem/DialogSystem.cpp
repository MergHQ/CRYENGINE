...
void CDialogSystem::ReleasePendingDeletes()
{
	if (m_pendingDeleteSessions.empty() == false)
	{
		...
		while (iter != end)
		{
			...
			CRY_ASSERT(m_allSessions.find((*iter)->GetSessionID()) == m_allSessions.end());
			(*iter)->Release();
			++iter;
		}
...
void CDialogSystem::Serialize(TSerialize ser)
{
	...
	else
	{
		...
		if (!CRY_VERIFY(tempSet.size() == temp.size()))
		{
		...
		CRY_ASSERT(nextSessionID >= m_nextSessionID);
