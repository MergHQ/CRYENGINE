	...
	: m_pCurLine(0)
{
	...
	m_SpeechAuxObject = Cry::Audio::InvalidAuxObjectId;
...
{
	if (m_SpeechAuxObject != Cry::Audio::InvalidAuxObjectId)
	{
...
bool CDialogActorContext::Update(float dt)
{
	...
	{
		if (IAIObject* aiObject = GetEntityAIObject(pEntity))
		{
	...
	do
	{
		...
		case eDAC_ScheduleSoundPlay:
			{
				bAdvance = true;
				const bool bHasSound = m_pCurLine->m_audioID != Cry::Audio::InvalidControlId;
				if (bHasSound)
				{
					if (m_bSoundScheduled == false)
					{
						...
						if (m_SpeechAuxObject == Cry::Audio::InvalidAuxObjectId)
						{
							...
							pActorAudioProxy->AddAsListenerToAudioAuxObject(m_SpeechAuxObject, &CDialogActorContext::OnAudioTriggerFinished, Cry::Audio::ESystemEvents::TriggerFinished);
						}
						UpdateAuxProxyPosition();

						Cry::Audio::SRequestUserData const userData(Cry::Audio::ERequestFlags::SubsequentCallbackOnExternalThread, nullptr, (void*)CDialogActorContext::GetClassIdentifier(), reinterpret_cast<void*>(static_cast<intptr_t>(m_ContextID)));
		...
		if (bAdvance)
		{
			...
			dt = 0.0f;
			++loop;
			if (!CRY_VERIFY(loop <= eDAC_EndLine + 1))
			{
...
{
	IAIObject* pAI = GetEntityAIObject(pEntity);
	if (pAI == 0)
		return false;
	...
	AISignals::IAISignalExtraData* pData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
...
bool CDialogActorContext::DoLookAt(IEntity* pEntity, IEntity* pLookAtEntity, bool& bReachedTarget)
{
	...
	{
		IAIObject* pTargetAI = GetEntityAIObject(pLookAtEntity);
...
bool CDialogActorContext::ExecuteAI(int& goalPipeID, const AISignals::ISignalDescription& signalDescription, AISignals::IAISignalExtraData* pExtraData, bool bRegisterAsListener)
{
	...
	goalPipeID = gEnv->pAISystem->Legacy()->AllocGoalPipeId();
	if (pExtraData == 0)
		pExtraData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
...
{
	return m_bSoundStarted && m_pCurLine && m_pCurLine->m_audioID != Cry::Audio::InvalidControlId; //m_soundID != INVALID_SOUNDID && m_bSoundStarted;
}
...
////////////////////////////////////////////////////////////////////////////
void CDialogActorContext::OnAudioTriggerFinished(Cry::Audio::SRequestInfo const* const pAudioRequestInfo)
{
...
////////////////////////////////////////////////////////////////////////////
CDialogActorContext* CDialogActorContext::GetDialogActorContextByAudioCallbackData(Cry::Audio::SRequestInfo const* const pAudioRequestInfo)
{
