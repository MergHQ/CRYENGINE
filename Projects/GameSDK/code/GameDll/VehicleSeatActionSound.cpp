...
CVehicleSeatActionSound::CVehicleSeatActionSound()
	...
	, m_pSeat(nullptr)
	, m_enabled(false)
	, m_audioTriggerStartId(Cry::Audio::InvalidControlId)
	, m_audioTriggerStopId(Cry::Audio::InvalidControlId)
{
}
...
bool CVehicleSeatActionSound::Init(IVehicle* pVehicle, IVehicleSeat* pSeat, const CVehicleParams& table)
{
	...
	m_audioTriggerStartId = Cry::Audio::StringToId(soundTable.getAttr("startTrigger"));
	m_audioTriggerStopId = Cry::Audio::StringToId(soundTable.getAttr("stopTrigger"));
...
void CVehicleSeatActionSound::ExecuteTrigger(const Cry::Audio::ControlId& controlID)
{
	if (controlID == Cry::Audio::InvalidControlId)
		return;
	...
	if (!gEnv->bMultiplayer && gEnv->pAISystem)
	{
		...
		{
			SAIStimulus stim(AISTIM_SOUND, AILegacy::AISOUND_MOVEMENT_LOUD, m_pVehicle->GetEntityId(), 0, m_pHelper->GetVehicleSpaceTranslation(), ZERO, 200.0f);
...
void CVehicleSeatActionSound::StopTrigger()
{
	...
	if (m_audioTriggerStopId != Cry::Audio::InvalidControlId)
	{
		ExecuteTrigger(m_audioTriggerStopId);
	}
	else if (m_audioTriggerStartId != Cry::Audio::InvalidControlId)
	{
		IEntityAudioComponent* pIEntityAudioComponent = m_pVehicle->GetEntity()->GetOrCreateComponent<IEntityAudioComponent>();
		CRY_ASSERT(pIEntityAudioComponent);
