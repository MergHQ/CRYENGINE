...
void CMuzzleEffect::MuzzleFlashEffect(CFireMode* pFireMode, int barrel)
{
	...
	bool shouldSendAIEvent = (muzzleFlash.aiVisibilityRadius > 0.0f) && (pWeapon->GetOwner() && pWeapon->GetOwner()->HasAI());
	if (shouldSendAIEvent)
	{
		if (gEnv->pAISystem->Legacy())
		{
			gEnv->pAISystem->Legacy()->DynOmniLightEvent(pWeapon->GetOwner()->GetWorldPos(),
				muzzleFlash.aiVisibilityRadius, AILegacy::AILE_MUZZLE_FLASH, pWeapon->GetOwnerId());
		}
	}
}
