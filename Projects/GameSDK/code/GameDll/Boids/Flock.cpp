...
namespace FlockConstants
{
	constexpr int PHYS_FOREIGN_ID_BOID = PHYS_FOREIGN_ID_USER - 1;
	constexpr float MAX_SPEED = 15;
	constexpr float MIN_SPEED = 2.5f;
	constexpr float ALIGNMENT_FACTOR = 1.0f;
	constexpr float COHESION_FACTOR = 1.0f;
	constexpr float SEPARATION_FACTOR = 10.0f;

	constexpr float ORIGIN_ATTRACT_FACTOR = 0.1f;
	constexpr float DESIRED_HEIGHT_FACTOR = 0.4f;
	constexpr float AVOID_LAND_FACTOR = 10.0f;

	constexpr float MAX_ANIMATION_SPEED = 1.7f;

	// previously hard-coded value in CBoidBird::ClampSpeed()
	constexpr float WALK_TO_IDLE_DURATION = 3.0f;

	// previously hard-coded values in CBoidBird::UpdateOnGroundAction()
	constexpr float ON_GROUND_WALK_DURATION_MIN = 4.0f;
	constexpr float ON_GROUND_WALK_DURATION_MAX = 6.0f;
	constexpr float ON_GROUND_IDLE_DURATION_MIN = 1.6f;
	constexpr float ON_GROUND_IDLE_DURATION_MAX = 3.2f;
};
...
{
	using namespace FlockConstants;
