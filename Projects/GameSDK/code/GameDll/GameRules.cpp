...
void CGameRules::OnVehicleEvent(IVehicle* pVehicle, EVehicleEvent event, const SVehicleEventParams& params)
{
	...
	if (!pAISystem || !pAISystem->IsEnabled() || !gEnv->pAISystem->Legacy())
		return;

	ISmartObjectManager* pSmartObjectManager = gEnv->pAISystem->Legacy()->GetSmartObjectManager();
	
	switch (event)
	{
	case eVE_Destroyed:
	{
		pSmartObjectManager->SetSmartObjectState(pVehicle->GetEntity(), "Dead");
		...
		for (int i = 0; i < pVehicle->GetWeaponCount(); ++i)
		{
			...
			{
				pSmartObjectManager->SetSmartObjectState(pWeaponEntity, "Busy");
			}
		}
		break;
	}
	case eVE_PassengerEnter:
	{
		...
		if (pActorEntity)
		{
			pSmartObjectManager->AddSmartObjectState(pActorEntity, "InVehicle");
		}
		break;
	}
	case eVE_PassengerExit:
	{
		...
		if (pActorEntity)
		{
			pSmartObjectManager->RemoveSmartObjectState(pActorEntity, "InVehicle");
		}
		break;
	}
	}
...
bool CGameRules::SetTeam_Common(int teamId, EntityId entityId, bool& bIsPlayer)
{
	...
	{	
		...
		CRY_ASSERT(pit!=m_playerteams.end());
...
void CGameRules::SetTeam(int teamId, EntityId entityId, bool clientOnly /*= false*/)
{
	if(!clientOnly)
	{
		if(!CRY_VERIFY(gEnv->bServer, "Cannot set team on the client"))
		{
...
{
	CRY_ASSERT((int)m_moduleRMIListenersVec.size() > index);
