...
public:
	...
	uint16 GetGameModeId(const Cry::Entity::IEntityClass *pClass) const;

	uint16 GetMapId(const char *pMapName) const;
	
	uint16 GetWeaponId(const char *pWeaponName) const;
	uint16 GetWeaponId(const Cry::Entity::IEntityClass *pClass) const;
...
private:
	...
	struct SClassId
	{
		const Cry::Entity::IEntityClass *m_pClass;
		uint16 m_id;
	};
