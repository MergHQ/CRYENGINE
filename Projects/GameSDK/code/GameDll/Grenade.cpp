...
void CGrenade::HandleEvent(const SGameObjectEvent &event)
{
	...
	if (event.event == eGFE_OnCollision)
	{
		...
		{
			IAIObject* pAI = GetEntityAIObject(GetEntity());
			if(pAI && pAI->GetAIType() == AIOBJECT_GRENADE)
			{
				...
				SAIStimulus stim(AISTIM_GRENADE, AILegacy::AIGRENADE_COLLISION, ownerId, GetEntityId(),
	...
	if(!gEnv->bMultiplayer && IPerceptionManager::GetInstance())
	{
		...
		if (pAI && pAI->GetAIType() == AIOBJECT_GRENADE)
		{
				...
				&& GetEntity()->GetPhysics()->GetStatus(&dynProj) && gEnv->pAISystem)
			{
				...
				SAIStimulus stim(AISTIM_GRENADE, AILegacy::AIGRENADE_THROWN, ownerId, GetEntityId(),
