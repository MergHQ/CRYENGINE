...
void CTacticalPointLanguageExtender::Initialize()
{
	...
	if (gEnv->pAISystem->Legacy() && gEnv->pAISystem->Legacy()->GetTacticalPointSystem())
	{
...
{
	if (gEnv->pAISystem && gEnv->pAISystem->Legacy() && gEnv->pAISystem->Legacy()->GetTacticalPointSystem())
	{
		UnregisterQueries();
...
bool CTacticalPointLanguageExtender::GeneratePoints(TGenerateParameters& parameters, SGenerateDetails& details, TObjectType object, const Vec3& objectPos, TObjectType auxObject, const Vec3& auxObjectPos) const
{
	...
	if (stricmp(parameters.query, "advantagePoints") == 0)
	{
		...
		IAIObjectManager* pAIObjMgr = gEnv->pAISystem->Legacy()->GetAIObjectManager();
...
bool CTacticalPointLanguageExtender::GetObject(TObjectParameters& parameters) const
{
	...
	if (!CRY_VERIFY(battleFrontGroup))
	{
	...
	if (!CRY_VERIFY(pBattleFrontAIObject))
			return false;
...
{
	CRY_ASSERT(gEnv->pAISystem->Legacy()->GetTacticalPointSystem() != NULL, "Expecting tactical point system to exist, but it doesn't.");
	ITacticalPointSystem& tacticalPointSystem = *gEnv->pAISystem->Legacy()->GetTacticalPointSystem();
...
{
	if (m_battlefrontAIObject != 0 && gEnv->pAISystem->Legacy())
	{
		ITacticalPointSystem& tacticalPointSystem = *gEnv->pAISystem->Legacy()->GetTacticalPointSystem();
