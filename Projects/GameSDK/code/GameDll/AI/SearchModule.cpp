...
{
	CRY_ASSERT(m_actors.empty());

	IAISystem& aiSystem = *gEnv->pAISystem;

	uint groupSize = (uint)aiSystem.Legacy()->GetGroupCount(groupID);
	m_actors.reserve(groupSize);

	for (uint i = 0; i < groupSize; ++i)
	{
		if (IAIObject* pAI = aiSystem.Legacy()->GetGroupMember(groupID, i))
...
{
	CRY_ASSERT(m_searchSpots.empty());
