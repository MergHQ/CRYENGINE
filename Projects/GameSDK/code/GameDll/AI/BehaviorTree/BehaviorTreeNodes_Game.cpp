...
namespace
{
	...
	IAIActor* GetIAIActor(const BehaviorTree::UpdateContext& updateContext)
	{
		...
		{
			if (IAIObject* ai = GetEntityAIObject(entity))
...
namespace BehaviorTree
{
	...
	private:
		...
		void ReleaseFormation(const UpdateContext& context)
		{
			...
			IAIObject* pIAIObject = GetEntityAIObject(pEntity);
			if (!pIAIObject)
				return;
...
#define MELEE_FRAGMENT_NAME "melee"
	...
	public:
		...
		{
			CRY_ASSERT(GetStatus() == Installed);
	...
	protected:
		virtual void Enter() override
		{
			...
			if (CRY_VERIFY(pAnimatedCharacter))
			{
		...
		struct RuntimeData : public IMeleeActionListener
		{
			...
			void ApplyDamage(const EntityId attackerId, IPhysicalEntity* pCollider, const Vec3& pos, const Vec3& normal, const int surfaceIdx, const int iPart, const float damages)
			{
				...
				if (pGameRules)
				{
					...
					CRY_ASSERT(collidedEntityId != attackerId);
			...
			bool PerformCylinderTest(const Vec3 &pos, const Vec3 &dir, const float hitRange)
			{
				...
				if( !CRY_VERIFY(pAttacker) )
					return false;

				IPhysicalEntity* pIgnore = pAttacker->GetPhysics();

				IAIObject* pAttackerAIObject = GetEntityAIObject(pAttacker);
				if( !CRY_VERIFY(pAttackerAIObject) )
					return false;
				...
				for (int i=0; i<totalIntersections; i++)
				{
					...
					if (contact)
					{
						...
						if (pCollider)
						{
							...
							if ( pEntity == NULL || pEntity == pAttacker || !pAttackerAIObject->IsHostile( GetEntityAIObject(pEntity) ) )
							{
								++currentContact;
								continue;
							}
	...
	protected:
		...
		virtual Status Update(const UpdateContext& context) override
		{
			...
			Agent agent(GetEntityAIObject(pEntity));
			IF_UNLIKELY(!agent.IsValid())
				return Success;
	...
	protected:	
		...
		{
			Agent agent(context.entityId);
			IF_UNLIKELY (!CRY_VERIFY(agent.IsValid()))
				return Running;
