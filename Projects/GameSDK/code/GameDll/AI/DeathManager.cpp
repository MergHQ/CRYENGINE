...
namespace GameAI
{
		...
		, m_rayID(0)
	{
		...
		if (gEnv->pAISystem->Legacy())
		{
			gEnv->pAISystem->Legacy()->RegisterSystemComponent(this);
			gEnv->pAISystem->Legacy()->Callbacks().AgentDied().Add(functor(*this, &DeathManager::OnAgentDeath));
		}
	}

	DeathManager::~DeathManager()
	{
		if (gEnv->pAISystem && gEnv->pAISystem->Legacy())
		{
			gEnv->pAISystem->Legacy()->Callbacks().AgentDied().Remove(functor(*this, &DeathManager::OnAgentDeath));
			gEnv->pAISystem->Legacy()->UnregisterSystemComponent(this);
		}
	}
	...
	void DeathManager::OnAgentGrabbedByPlayer(const EntityId agentID)
	{
		...
		{
			if (IAIObject* pAI = GetEntityAIObject(pObjectEntity))
			{
				if (pAI->CastToIAIActor() != nullptr)
				{
					...
					gEnv->pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_GROUPONLY_EXCEPT, pSignal);
				}
			}
		}
	}
	...
	void DeathManager::AddDeferredDeathReactionFor(const Agent& deadAgent, const EntityId killerID)
	{
		...
		int memberCount = aiSystem.Legacy()->GetGroupCount(groupID);
		if (memberCount == 0)
			return;
		...
		{
			Agent member(aiSystem.Legacy()->GetGroupMember(groupID, memberIndex));
	...
	void DeathManager::DispatchDeferredDeathReaction(const DeferredDeathReaction& ddr)
	{
		...
		if (arbitraryMember)
		{
			if (IAIGroupProxy* group = aiSystem.Legacy()->GetAIGroupProxy(ddr.groupID))
			{
				...
				aiSystem.GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_GROUPONLY_EXCEPT, pSignal);
			}
		}
	...
	{
		CRY_ASSERT(m_rayID == 0);
		CRY_ASSERT(m_asyncState == AsyncReady);
	...
	void DeathManager::OnAgentDeath(EntityId deadEntityID, EntityId killerID)
	{
		...
		if (deadAgent)
		{
			const bool lastOneInGroupJustDied = (gEnv->pAISystem->Legacy()->GetGroupCount(deadAgent.GetGroupID()) == 0);
	...
	{
		CRY_ASSERT(m_asyncState == AsyncInProgress);
		CRY_ASSERT(m_rayID == rayID);

		m_asyncState = AsyncReady;
		m_rayID = 0;

		if (CRY_VERIFY(!m_deferredDeathReactions.empty()))
		{
			DeferredDeathReaction& ddr = m_deferredDeathReactions.front();
