...
void CVisibleObjectsHelper::Update()
{
	...
	IAIObjectManager* pAIObjectManager = !activeVisibleObjects.empty() && gEnv->pAISystem->Legacy() ? gEnv->pAISystem->Legacy()->GetAIObjectManager() : NULL;
		...
		if (bVisible)
		{
			...
			if (pAIEntity != NULL && eVOR_FlagNotifyOnSeen == (visibleObject->rule & eVOR_FlagNotifyOnSeen))
			{
				...
				if (pObservableParams != NULL)
				{
					...
					ITargetTrackManager *pTargetTrackManager = gEnv->pAISystem->Legacy() ? gEnv->pAISystem->Legacy()->GetTargetTrackManager() : nullptr;
					if (pTargetEntity != NULL && pTargetTrackManager != NULL)
					{
						const tAIObjectID aiOwnerId = GetEntityAIObjectID(pAIEntity);
						const tAIObjectID aiTargetId = GetEntityAIObjectID(pTargetEntity);
						...
						IAIObject* pAIObjectSender = GetEntityAIObject(pAIEntity);
						if (pAIObjectSender)
						{
							AISignals::IAISignalExtraData *pSignalData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
							if (pSignalData)
							{
							...
							gEnv->pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_SENDER, pSignal);
						}
					}
				}
			}
