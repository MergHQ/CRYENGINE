...
void CGameAIRecorder::Init()
{
	...
	IAIRecorder *pRecorder = gEnv->pAISystem->Legacy() ? gEnv->pAISystem->Legacy()->GetIAIRecorder() : nullptr;
	if (pRecorder)
	{
...
void CGameAIRecorder::AddRecordBookmark(EntityId requesterId)
{
	...
	if (m_bIsRecording)
	{
		...
		AILegacy::IAIObject *pAI = pEntity ? GetEntityAIObject(pEntity) : NULL;
		if (!pAI)
		{
...
void CGameAIRecorder::AddRecordComment(EntityId requesterId, const char* szComment)
{
	...
	if (m_bIsRecording && CRY_VERIFY(szComment && szComment[0]))
	{
		if (!gEnv->bServer)
		{
...
{
	if (m_bIsRecording && CRY_VERIFY(agent.IsValid()))
	{
		char szBuffer[1024];
...
bool CGameAIRecorder::AddFileToRemoteArchive(const char* szFile)
{
	...
	if (m_pRemoteArchive && CRY_VERIFY(pCryPak))
	{
