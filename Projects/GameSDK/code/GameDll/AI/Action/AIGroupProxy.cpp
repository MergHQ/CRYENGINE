...
void CAIGroupProxy::Notify(uint32 notificationID, const AISignals::SignalSharedPtr& pSignal)
{
	...
	for (; it != end; ++it)
	{
		if (IAIObject* aiObject = gEnv->pAISystem->Legacy()->GetAIObjectManager()->GetAIObject(*it))
		{
			...
			gEnv->pAISystem->GetSignalManager()->SendSignal(AISignals::SIGNALFILTER_SENDER, pNewSignal);
		}
	}
}
