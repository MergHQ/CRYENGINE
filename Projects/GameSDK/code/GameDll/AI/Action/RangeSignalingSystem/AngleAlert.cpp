...
{
	if (gEnv->pAISystem)
		gEnv->pAISystem->GetSignalManager()->FreeSignalExtraData(m_pSignalData);
}
...
void CAngleAlert::Init(float fAngle, float fBoundary, const char* sSignal, AISignals::IAISignalExtraData* pData /*= NULL*/)
{
	...
	{
		gEnv->pAISystem->GetSignalManager()->FreeSignalExtraData(m_pSignalData);
		m_pSignalData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
...
float CAngleAlert::GetAngleTo(const Vec3& vPos) const
{
	...
	if (pEntity)
	{
		...
		const Vec3& vEntityDir = GetEntityAIObject(pEntity)->GetViewDir();
