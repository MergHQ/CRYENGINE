...
void CAIHandler::AIMind(SOBJECTSTATE& state)
{
	...
	AISignals::ISignalManager* pSignalsManager = gEnv->pAISystem->GetSignalManager();
	IAIObject* pEntityAI = GetEntityAIObject(m_pEntity);
	...
	case AITARGET_MEMORY:
		...
		if (prevType != AITARGET_MEMORY)
		{
			if (state.eTargetThreat == AITHREAT_THREATENING)
			{
				...
				pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnEnemyMemory();
			}
			else if (state.eTargetThreat == AITHREAT_AGGRESSIVE)
			{
				value = &state.fDistanceFromTarget;
				pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnLostSightOfTarget();
			}
		}
		else
		{
			...
			{
				value = &state.fDistanceFromTarget;
				pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnMemoryMoved();
			}
		}
	...
	case AITARGET_SOUND:
		...
		{
			value = &state.fDistanceFromTarget;
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnEnemyHeard();
		}
		else if (state.eTargetThreat == AITHREAT_THREATENING)
		{
			value = &state.fDistanceFromTarget;
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnThreateningSoundHeard();
		}
		else if (state.eTargetThreat == AITHREAT_INTERESTING)
		{
			value = &state.fDistanceFromTarget;
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnInterestingSoundHeard();
		}
		else if (state.eTargetThreat == AITHREAT_SUSPECT)
		{
			value = &state.fDistanceFromTarget;
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnSuspectedSoundHeard();
		}
		break;
	case AITARGET_VISUAL:
		...
		if (state.nTargetType > AIOBJECT_PLAYER)   //-- grenade (or any other registered object type) seen
		{
			...
			value = &state.fDistanceFromTarget;
			if (!pExtraData)
				pExtraData = pSignalsManager->CreateSignalExtraData();
			...
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnObjectSeen();
		}
		else if (state.eTargetThreat == AITHREAT_AGGRESSIVE)
		{
			if (!pExtraData)
			...
			if (IAIActor* pAIActor = CastToIAIActorSafe(pEntityAI))
			{
				...
				if (pTargetActor && pTargetActor->CanAcquireTarget(pEntityAI))
				{
					...
					{
						pSignal = gEnv->pAISystem->GetSignalManager()->CreateSignal(AISIGNAL_DEFAULT, pSignalsManager->GetBuiltInSignalDescriptions().GetOnSeenByEnemy(), pTarget->GetEntityID());
						pSignalsManager->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_SENDER, pSignal);
					}
				}
			}
			...
			if (targetIsCloaked)
			{
				pSignal = pSignalsManager->CreateSignal(AISIGNAL_DEFAULT, pSignalsManager->GetBuiltInSignalDescriptions().GetOnCloakedTargetSeen(), 0, pExtraData);
				AISignal(pSignal);
			}
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnEnemySeen();
		}
		else if (state.eTargetThreat == AITHREAT_THREATENING)
		{
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnThreateningSeen();
		}
		else if (state.eTargetThreat == AITHREAT_INTERESTING)
		{
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnSomethingSeen();
		}
		else if (state.eTargetThreat == AITHREAT_SUSPECT)
		{
			pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnSuspectedSeen();
		}
		break;
	case AITARGET_NONE:
		pSignalDescription = &pSignalsManager->GetBuiltInSignalDescriptions().GetOnNoTarget();
		break;
	}

	if (pSignalDescription && !pSignalDescription->IsNone())
	{
		pSignal = pSignalsManager->CreateSignal(AISIGNAL_DEFAULT, *pSignalDescription, 0, pExtraData);

		gEnv->pAISystem->Legacy()->Record(pEntityAI, IAIRecordable::E_HANDLERNEVENT, pSignal->GetSignalDescription().GetName());
	...
	if (pExtraData)
	{
		pSignalsManager->FreeSignalExtraData(pExtraData);
		if (pSignal)
		{
...
void CAIHandler::AISignal(AISignals::SignalSharedPtr pSignal)
{
	...
	{
		IAIRecordable::RecorderEventData recorderEventData(pSignal->GetSignalDescription().GetName());
		GetEntityAIObject(m_pEntity)->RecordEvent(IAIRecordable::E_SIGNALEXECUTING, &recorderEventData);
		gEnv->pAISystem->Legacy()->Record(GetEntityAIObject(m_pEntity), IAIRecordable::E_SIGNALEXECUTING, pSignal->GetSignalDescription().GetName());
	}
	...
	if (!CallScript(m_pBehavior, pSignal->GetSignalDescription().GetName(), NULL, pSender, pSignal->GetExtraData()))
	{
		...
		if (!CallScript(m_pDefaultBehavior, pSignal->GetSignalDescription().GetName(), NULL, pSender, pSignal->GetExtraData()))
		{
			...
			{
				gEnv->pAISystem->Record(GetEntityAIObject(m_pEntity), IAIRecordable::E_SIGNALEXECUTING, "from defaultDefault behaviour");
			}
		}
		else
		{
			gEnv->pAISystem->Record(GetEntityAIObject(m_pEntity), IAIRecordable::E_SIGNALEXECUTING, "from default behaviour");
		}
#else
		if (CallScript(m_pDEFAULTDefaultBehavior, pSignal->GetSignalDescription().GetName(), NULL, pSender, pSignal->GetExtraData()))
		{
			gEnv->pAISystem->Legacy()->Record(GetEntityAIObject(m_pEntity), IAIRecordable::E_SIGNALEXECUTING, "from defaultDefault behaviour");
		}
#endif
	}
	...
	if (m_pEntity)
	{
		...
		CRY_ASSERT(GetEntityAIObject(m_pEntity) != NULL);
		IAIActor* aiActor = GetEntityAIObject(m_pEntity)->CastToIAIActor();
		if (aiActor != NULL)
		{
...
void CAIHandler::ResetBehavior()
{
	...
	if (m_pBehavior.GetPtr())
	{
		...
		gEnv->pAISystem->Legacy()->Record(GetEntityAIObject(m_pEntity), IAIRecordable::E_BEHAVIORDESTRUCTOR, m_sBehaviorName);
	}
...
void CAIHandler::SetBehavior(const char* szNextBehaviorName, const AISignals::IAISignalExtraData* pData, ESetFlags setFlags)
{
	...
	IAIObject* aiObject = GetEntityAIObject(m_pEntity);
	...
	if (setFlags != SET_ON_SERILIAZE)
	{
		if (m_pBehavior.GetPtr())
		{
			...
			gEnv->pAISystem->Legacy()->Record(aiObject, IAIRecordable::E_BEHAVIORDESTRUCTOR, m_sBehaviorName);
	...
	gEnv->pAISystem->Legacy()->Record(aiObject, IAIRecordable::E_BEHAVIORSELECTED, m_sBehaviorName);
	...
	if (m_pBehavior.GetPtr())
	{
		...
		{
			if (IPipeUser* pipeUser = CastToIPipeUserSafe(GetEntityAIObject(m_pEntity)))
...
void CAIHandler::CallBehaviorConstructor(const AISignals::IAISignalExtraData* pData)
{
	if (m_pBehavior.GetPtr())
	{
		gEnv->pAISystem->Legacy()->Record(GetEntityAIObject(m_pEntity), IAIRecordable::E_BEHAVIORCONSTRUCTOR, m_sBehaviorName);

		IAIActor* pActor = GetEntityAIObject(m_pEntity)->CastToIAIActor();
		if (pActor != NULL)
		{
		...
		if (bFound)
		{
			...
			if (g_pGame->GetCVars()->aiLogSignals)
			{
				...
				if (start.GetMilliSeconds() > g_pGame->GetCVars()->aiMaxSignalDuration)
				{
					...
					gEnv->pAISystem->Record(m_GetEntityAIObject(pEntity), IAIRecordable::E_SIGNALEXECUTEDWARNING, buffer);
				}
			}
#endif
...
void CAIHandler::SetAlertness(int value, bool triggerEvent /*=false*/)
{
	...
	if (sStates)
		gEnv->pAISystem->Legacy()->GetSmartObjectManager()->ModifySmartObjectStates(m_pEntity, sStates);
}
...
CAnimActionExactPositioning* CAIHandler::CreateExactPositioningAction(bool isOneShot, const float loopDuration, const char* szFragmentID, bool isNavigationalSO, const QuatT& exactStartLocation)
{
	...
	if (!CRY_VERIFY(valueCRC, "no signal or action was selected, or we're extremely unlucky and hit a 0 CRC"))
	{
		return NULL;
	}
...
void CAIHandler::HandleExactPositioning(SOBJECTSTATE& state, CMovementRequest& mr)
{
	...
	if (m_eActorTargetPhase == eATP_Waiting)
	{
		//		if(state.actorTargetPhase == eATP_Waiting)
		{
			...
			if (!CRY_VERIFY(animTgt, "exact positioning request was made but was invalid or cleared after some time without any notification"))
			{
...
{
	IAIObject* pAI = GetEntityAIObject(m_pEntity);
	...
	int count = pAISystem->Legacy()->GetGroupCount(groupId, AILegacy::GROUP_ENABLED, nType);
	float nearestDist = FLT_MAX;
	IAIObject* nearestInGroup = 0;

	for (int i = 0; i < count; ++i)
	{
		IAIObject* responder = pAISystem->Legacy()->GetGroupMember(groupId, i, AILegacy::GROUP_ENABLED, nType);
	...
	if (nearestInGroup)
	{
		AISignals::IAISignalExtraData* pData = pAISystem->GetSignalManager()->CreateSignalExtraData();
		...
		pAISystem->GetSignalManager()->SendSignal(AISignals::ESignalFilter::SIGNALFILTER_READABILITYRESPONSE, pSignal);
	}
}
...
bool CAIHandler::SetMannequinAGInput(IActionController* pActionController, EAIAGInput input, const char* value, const bool isUrgent)
{
	...
	if (input == AIAG_ACTION)
	{
		...
		else
		{
			if (!isIdleValue)
			{
				//if (IPipeUser* pPipeUser = CastToIPipeUserSafe(m_GetEntityAIObject(pEntity)))
				//{
