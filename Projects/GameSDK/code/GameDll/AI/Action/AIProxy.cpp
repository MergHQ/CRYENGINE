...
IWeapon* CAIProxy::GetSecWeapon(const ERequestedGrenadeType prefGrenadeType, ERequestedGrenadeType* pReturnedGrenadeType, EntityId* const pSecondaryWeaponId) const
{
	...
	IAIObject* pAI = GetEntityAIObject(m_pGameObject->GetEntity());
...
bool CAIProxy::CheckUpdateStatus()
{
	...
	// (this will result in an infinite-ish loop)
	if (gEnv->pAISystem->Legacy()->GetUpdateAllAlways() || m_UpdateAlways)
...
void CAIProxy::UpdateShooting(const SOBJECTSTATE& state, bool isAlive)
{
	...
	if (pEntity)
	{
		IPuppet* pPuppet = CastToIPuppetSafe(GetEntityAIObject(pEntity));
		if (pPuppet)
	...
	if (eAIFS_On == state.fireMelee)
	{
		if (isAlive && (pWeapon != NULL) && hasFireControl)
		{
			...
			if (IAIObject* aiObject = GetEntityAIObject(m_pAIHandler->m_pEntity))
			{
...
void CAIProxy::SetForcedExecute(bool forced)
{
	...
	CRY_ASSERT(m_forcedExecute >= 0); // Below zero? More disables then enables!
	CRY_ASSERT(m_forcedExecute < 16); // Just a little sanity check

	if (IEntity* pEntity = m_pGameObject->GetEntity())
	{
		IAIActor* pAIActor = CastToIAIActorSafe(GetEntityAIObject(pEntity));
		if (pAIActor)
		{
			if (forced)
...
void CAIProxy::PlayAnimationAction(const IAIAction* pAction, int goalPipeId)
{
	...
	if (m_animActionGoalPipeId != 0)
		gEnv->pAISystem->Legacy()->GetAIActionManager()->AbortAIAction(m_pGameObject->GetEntity(), m_animActionGoalPipeId);
...
void CAIProxy::AnimationActionDone(bool succeeded)
{
	if (m_animActionGoalPipeId)
	{
		...
		m_animActionGoalPipeId = 0;
		if (succeeded)
			gEnv->pAISystem->Legacy()->GetAIActionManager()->FinishAIAction(m_pGameObject->GetEntity(), tmp);
		else
			gEnv->pAISystem->Legacy()->GetAIActionManager()->AbortAIAction(m_pGameObject->GetEntity(), tmp);
	}
}
...
void CAIProxy::UpdateMind(SOBJECTSTATE& state)
{
	...
	if (GetEntityAIObject(m_pGameObject->GetEntity())->IsUpdatedOnce())
	{
		while (!state.vSignals.empty())
		{
			...
			else
			{
				...
				{
					gEnv->pAISystem->GetSignalManager()->FreeSignalExtraData(pSignalFront->GetExtraData());
		...
		while (i < size)
		{
			...
			if (pSignal->GetNSignal() == AISIGNAL_INITIALIZATION)
			{
				...
				if (pSignal->GetExtraData())
					gEnv->pAISystem->GetSignalManager()->FreeSignalExtraData(pSignal->GetExtraData());
