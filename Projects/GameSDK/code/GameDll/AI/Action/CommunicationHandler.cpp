...
void SetSwitchState(const char* switchIdName, const char* switchValue, IEntityAudioComponent* pIEntityAudioComponent)
{
	...
	Cry::Audio::ControlId const switchId = Cry::Audio::StringToId(switchIdName);
	Cry::Audio::ControlId const switchStateId = Cry::Audio::StringToId(switchValue);
	...
	, m_actionInputID(0)
{
	...
	gEnv->pAudioSystem->AddRequestListener(&CommunicationHandler::TriggerFinishedCallback, this, Cry::Audio::ESystemEvents::TriggerFinished);
}
...
void CommunicationHandler::Reset()
{
	...
	if (pEntity)
	{
		...
		if (pIEntityAudioComponent)
		{
			const ICommunicationManager::WWiseConfiguration& wiseConfigutation = gEnv->pAISystem->Legacy()->GetCommunicationManager()->GetWiseConfiguration();
			...
			{
				if (IAIObject* pAIObject = GetEntityAIObject(pEntity))
				{
...
void CommunicationHandler::StopSound(const SCommunicationSound& soundToStop)
{
	...
	if (pEntity)
	{
		...
		if (pIEntityAudioComponent)
		{
			if (soundToStop.stopSoundControlId != Cry::Audio::InvalidControlId)
			{
...
SCommunicationSound CommunicationHandler::PlaySound(CommPlayID playID, const char* name, ESoundType type, IEventListener* listener)
{
	...
	if (pEntity)
	{
		...
		if (pIEntityAudioComponent)
		{
			...
			Cry::Audio::ControlId const playCommunicationControlId = Cry::Audio::StringToId(playTriggerName.c_str());
			Cry::Audio::ControlId const stopCommunicationControlId = Cry::Audio::StringToId(stopTriggerName.c_str());
			...
			Cry::Audio::SRequestUserData const userData(Cry::Audio::ERequestFlags::SubsequentCallbackOnExternalThread, this, reinterpret_cast<void*>(static_cast<UINT_PTR>(m_entityId)), this);
			pIEntityAudioComponent->ExecuteTrigger(playCommunicationControlId, Cry::Audio::DefaultAuxObjectId, INVALID_ENTITYID, userData);
...
void CommunicationHandler::TriggerFinishedCallback(Cry::Audio::SRequestInfo const* const pAudioRequestInfo)
{
	...
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(entityId))
	{
		if (IAIObject* pAIObject = GetEntityAIObject(pEntity))
		{
			if (IAIActorProxy* pAiProxy = pAIObject->GetProxy())
			{
				...
				{
					pCommunicationHandler->OnSoundTriggerFinishedToPlay(pAudioRequestInfo->controlId);
				}
			}
		}
	}
...
void CommunicationHandler::OnSoundTriggerFinishedToPlay(Cry::Audio::ControlId const triggerId)
{
