...
void CPersonalSignalTimer::SendSignal()
{
	...
	if (pEntity && gEnv->pAISystem)
	{
		...
		AISignals::IAISignalExtraData* pData = pSignalsManager->CreateSignalExtraData();
		pData->iValue = ++m_iSignalsSinceLastReset;
		pData->fValue = m_fTimerSinceLastReset;

		const AISignals::SignalSharedPtr pSignal = pSignalsManager->CreateSignal_DEPRECATED(AISIGNAL_DEFAULT, m_sSignal, pEntity->GetId(), pData);
