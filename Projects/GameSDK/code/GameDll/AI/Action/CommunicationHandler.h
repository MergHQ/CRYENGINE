...
public:
	...
	virtual void                OnSoundTriggerFinishedToPlay(Cry::Audio::ControlId const triggerId) override;
	//~IAICommunicationHandler
	...
	static void TriggerFinishedCallback(Cry::Audio::SRequestInfo const* const pAudioRequestInfo);
...
private:
	...
	struct PlayingSound
	{
		PlayingSound()
			...
			, type(Sound)
			, playID(0)
			, correspondingStopControlId(Cry::Audio::InvalidControlId)
		{
		}
		...
		CommPlayID                               playID;
		Cry::Audio::ControlId                      correspondingStopControlId;
	};
	...
	typedef std::map<Cry::Audio::ControlId, PlayingSound> PlayingSounds;
