...
{		
	// Sanity checks.
	CRY_ASSERT(hazardContext.m_Pos.IsValid());
	CRY_ASSERT(hazardContext.m_MoveNormal.IsValid());
	CRY_ASSERT(hazardContext.m_MoveNormal.IsUnit());
	CRY_ASSERT(hazardContext.m_Radius >= 0.0f);	
	CRY_ASSERT(hazardContext.m_MaxScanDistance >= 0.0f);	
	CRY_ASSERT(hazardContext.m_MaxPosDeviationDistance >= 0.0f);
	CRY_ASSERT(hazardContext.m_MaxAngleDeviationRad >= 0.0f);
...
{
	// Sanity checks.
	CRY_ASSERT(newNormal.IsValid());
	CRY_ASSERT(newNormal.IsUnit());

	int containerIndex = IndexOfInstanceID(m_ProjectileHazards, hazardInstanceID.GetHazardID());
	if (!CRY_VERIFY(containerIndex >= 0))
	{
		return false;
	}
...
{		
	if (!CRY_VERIFY(m_running.get() != NULL))
	{
		return;
	}
...
{
	AISignals::IAISignalExtraData* extraData = gEnv->pAISystem->GetSignalManager()->CreateSignalExtraData();
