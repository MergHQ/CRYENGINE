...
struct InstanceInitContext
{
	InstanceInitContext(Instance* _instance, InstanceID _instanceID, IEntity* _entity, EntityId _entityID)
		...
		, entityID(_entityID)
	{
		CRY_ASSERT(_instance);
		CRY_ASSERT(_instanceID != InstanceID::Invalid());
		CRY_ASSERT(_entity);
		CRY_ASSERT(_entityID != 0);
	}
...
public:
	...
	Instance* GetInstanceFromID(InstanceID id)
	{
		...
		if (CRY_VERIFY((offset + sizeof(Instance)) <= m_pool.size()))
		{
