...
CProjectile::CProjectile()
	: m_whizTriggerID(Cry::Audio::InvalidControlId),
	m_ricochetTriggerID(Cry::Audio::InvalidControlId),
...
bool CProjectile::Init(IGameObject* pGameObject)
{
	...
	if (m_pAmmoParams->pRicochet)
	{
		...
		{
			m_ricochetTriggerID = Cry::Audio::StringToId(ricochetTriggerName.c_str());
		}
	}
	...
	if (m_pAmmoParams->pWhiz)
	{
		...
		{
			m_whizTriggerID = Cry::Audio::StringToId(whizTriggerName.c_str());
		}
	}
...
bool CProjectile::ProcessCollisionEvent(IEntity* pTarget) const
{
	...
	if ((!bEnableFriendlyHit) && (!gEnv->bMultiplayer))
	{
		...
		IAIObject* pOwnerAI = pOwner ? GetEntityAIObject(pOwner) : nullptr;
		IAIObject* pTargetAI = pTarget ? GetEntityAIObject(pTarget) : nullptr;
...
void CProjectile::Launch(const Vec3& pos, const Vec3& dir, const Vec3& velocity, float speedScale /*=1.0f*/)
{
	...
	if (pOwnerEntity)
	{
		...
		if (pSelfEntity)
		{
			...
			IAIObject* projectileAI = GetEntityAIObject(pSelfEntity);
			IAIObject* shooterAI = GetEntityAIObject(pOwnerEntity);
...
void CProjectile::Destroy()
{
	...
	if (GetEntity()->IsKeptAlive() || delayedDestruct)
	{
		...
		gEnv->pAISystem->Legacy()->GetAIObjectManager()->RemoveObjectByEntityId(GetEntityId()); // unregister from AI. Will be removed from active list when hidden otherwise (see EvaluateUpdateActivation)
...
{
	if (m_pAmmoParams->pWhiz && m_whizTriggerID != Cry::Audio::InvalidControlId && !IsEquivalent(m_last, pos))
	{
...
{
	Cry::Audio::SExecuteTriggerData const data(m_whizTriggerID, "WhizBy", Cry::Audio::EOcclusionType::Ignore, pos, INVALID_ENTITYID, true);
...
{
	Cry::Audio::SExecuteTriggerData const data(m_ricochetTriggerID, "Ricochet", Cry::Audio::EOcclusionType::Ignore, pos, INVALID_ENTITYID, true);
	gEnv->pAudioSystem->ExecuteTriggerEx(data);
}
...
void CProjectile::FlashbangEffect(const SFlashbangParams* flashbang)
{
	...
	SAIStimulus stim(AISTIM_GRENADE, AILegacy::AIGRENADE_FLASH_BANG, ownerId, GetEntityId(),
	                 GetEntity()->GetWorldPos(), ZERO, radius);
	pPerceptionManager->RegisterStimulus(stim);

	SAIStimulus stimSound(AISTIM_SOUND, AILegacy::AISOUND_WEAPON, ownerId, 0,
...
void CProjectile::InitWithAI()
{
	...
	if (m_pAmmoParams->aiType != AIOBJECT_NONE)
	{
		...
		{
			if (IAIObject* pOwnerAI = GetEntityAIObject(pOwnerEntity))
			{
				unsigned short int nOwnerType = pOwnerAI->GetAIType();
				if (nOwnerType != AIOBJECT_ACTOR)
				{
					gEnv->pAISystem->Legacy()->GetAIObjectManager()->CreateAIObject(AIObjectParams(m_pAmmoParams->aiType, 0, GetEntityId()));
				}
			}
		}
	}
...
bool CProjectile::ProximityDetector_SP(float proxyRadius)
{
	...
	for (int j = 0; j < query.nCount; ++j)
	{
		...
		IAIObject* pAIObject = GetEntityAIObject(pEntity);
