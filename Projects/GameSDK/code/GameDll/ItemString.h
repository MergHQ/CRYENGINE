...
namespace SharedString
{
	...
	private:
		...
		SNameEntry* _entry( const char *pBuffer ) const { CRY_ASSERT(pBuffer); return ((SNameEntry*)pBuffer)-1; }
	...
	{
		//CRY_ASSERT(s); // AlexL: we currenly allow 0 assignment because Items currently do this a lot
		const char *pBuf = 0;
		if (s && *s) // if not empty
		{
