...
CGame::~CGame()
{
	...
	{
		Cry::DRS::IResponseActor* pDrsActor = gEnv->pDynamicResponseSystem->GetResponseActor("GameTokenSignalSender");
		if (pDrsActor)
		{
...
bool CGame::Init(/*IGameFramework* pFramework*/)
{
	...
	if (gEnv->pDynamicResponseSystem)
	{
		...
		Cry::DRS::IResponseActor* pDrsActor = gEnv->pDynamicResponseSystem->CreateResponseActor("GameTokenSignalSender");
...
bool CGame::CompleteInit()
{
	...
	CRY_ASSERT(gEnv->bMultiplayer || m_pGameAISystem);
	if (m_pGameAISystem)
	{
...
int CGame::Update(bool haveFocus, unsigned int updateFlags) PREFAST_SUPPRESS_WARNING(6262)
{
	...
	{
		CRY_ASSERT(gEnv->bMultiplayer);
		CRY_ASSERT(m_allowedToLoadMultiplayerFrontEndAssets);
		...
		{
			{
				...
				else
				{
					...
					if (event.m_uParams.m_signIn.m_previousSignedInState != IPlatformOS::SPlatformEvent::eSIS_NotSignedIn)
					{
						...
						if (CRY_VERIFY(bIsExclusiveController || event.m_user == IPlatformOS::Unknown_User))
						{
	...
	case ESYSTEM_EVENT_EDITOR_GAME_MODE_CHANGED:
		{
			if (wparam != 0)
			{
				...
				if (pLocalPlayerEntity != NULL)
				{
					...
					{
						pIEntityAudioComponent->SetCurrentEnvironments(Cry::Audio::InvalidAuxObjectId);
					}
				}
			}
...
int CGame::GetCheckpointIDForTelemetry(const char* checkpointName) const
{
	...
	CRY_ASSERT(!"invalid checkpoint name in CGame::GetCheckpointIDForTelemetry()");
	return 0;
}
