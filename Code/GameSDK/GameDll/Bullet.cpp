...
void SDebugBulletPenetration::Update(float frameTime)
{
	...
	for (int i = 0; i < MAX_DEBUG_BULLET_HITS; ++i)
	{
		...
		IRenderAuxText::DrawLabelEx(baseText - (textLineOffset * 2.0f), 1.25f, textColor, true, false, "Damage: %.1f", currentHit.damage);
		IRenderAuxText::DrawLabelEx(baseText - (textLineOffset * 3.0f), 1.25f, textColor, true, false, "Pierceability: %d", currentHit.surfacePierceability);
		IRenderAuxText::DrawLabelEx(baseText - (textLineOffset * 4.0f), 1.25f, textColor, true, false, "%s", GetPenetrationLevelByPierceability(currentHit.surfacePierceability));
