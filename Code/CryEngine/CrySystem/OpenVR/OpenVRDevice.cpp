...
void Device::UpdateTrackingState(EVRComponent type)
{
	...
	m_system->GetDeviceToAbsoluteTrackingPose(CVars::hmd_reference_point == 1 ? vr::ETrackingUniverseOrigin::TrackingUniverseStanding : vr::ETrackingUniverseOrigin::TrackingUniverseSeated, fSecondsUntilPhotons, m_rTrackedDevicePose, vr::k_unMaxTrackedDeviceCount);
...
// -------------------------------------------------------------------------
void Device::OnCommand(CryVR::EVRCmd cmd, IConsoleCmdArgs* pArgs)
{
	switch (cmd)
	{
