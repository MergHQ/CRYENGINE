...
namespace CryMT
{
	...
	private:
		...
		void CheckForTombstoneInsert(uint32 index) const;
	...
	inline void CThreadSafePushContainer<T>::push_back(const T& rObj)
	{
		...
		::new(pObj) T(rObj);
		CheckForTombstoneInsert(index); // check that pushed object is not a tombstone
	}
	...
	template<typename T>
	void CThreadSafePushContainer<T>::CheckForTombstoneInsert(uint32 index) const
	{
