...
template<typename F> struct Vec4_tpl
{
	...
	template<class T> ILINE Vec4_tpl(const Vec4_tpl<T>& v) : x((F)v.x), y((F)v.y), z((F)v.z), w((F)v.w) { CRY_MATH_ASSERT(this->IsValid()); }
...
typedef               CRY_ALIGN (16) Vec4_tpl<f32> Vec4A;
#endif
