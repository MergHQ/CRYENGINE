...
#define VALIDATE_CHUCKS
	...
	// Initial data arrived, mark all corresponding REs dirty to trigger necessary recompiles
...
void CRenderMesh::CopyTo(IRenderMesh* _pDst, int nAppendVtx, bool bDynamic, bool fullCopy)
{
	...
	// Copied data inserted, mark all corresponding REs dirty to trigger necessary recompiles
	pDst->MarkRenderElementsDirty();
...
bool CRenderMesh::RT_CheckUpdate(CRenderMesh* pVContainer, EVertexFormat eVF, uint32 nStreamMask, bool bTessellation, bool stall)
{
	...
	for (size_t i = 0, end = pVContainer->m_CreatedBoneIndices[threadId].size(); i < end; ++i)
	{
		...
		if (bFound == false)
		{
			...
			stream.buffer = gRenDev->m_DevBufMan.Create(BBT_VERTEX_BUFFER, BU_STATIC, pVContainer->GetVerticesCount() * pVContainer->GetStreamStride(VSF_HWSKIN_INFO));
			stream.guid = guid;
...
void CRenderMesh::MarkRenderElementsDirty()
{
	...
	for (auto& chunk : m_Chunks)
	{
		if (chunk.pRE)
		{
			...
			//{ char buf[1024]; cry_sprintf(buf, "    RE: %p : frame(%d) set FCEF_DIRTY\r\n", chunk.pRE, nFrameId); OutputDebugString(buf); }
		}
	}
}
...
void CRenderMesh::BindStreamsToRenderPipeline()
{
...
#ifdef MESH_TESSELLATION_RENDERER
	if (CHWShader_D3D::s_pCurInstHS && pRenderMesh->m_adjBuffer.GetSRV())
	{
	...
	if (auto pSrv = pRenderMesh->m_extraBonesBuffer.GetSRV())
	{
...
bool CRenderMesh::FillGeometryInfo(CRendElementBase::SGeometryInfo& geom)
{
	...
	for (int nStream = 0; nStream < VSF_NUM; ++nStream)
	{
		...
		{
			nOffs = 0;
			geom.vertexStream[nStream].hStream = pRenderMeshForVertices->_GetVBStream(nStream);
			geom.vertexStream[nStream].nStride = pRenderMeshForVertices->GetStreamStride(nStream);

			if (geom.vertexStream[nStream].hStream)
			{
				geom.nNumVertexStreams = nStream + 1;
			}
		...
		{
			geom.vertexStream[nStream].hStream = ~0u;
	...
	{
		if (geom.nNumVertexStreams <= VSF_HWSKIN_INFO)
		{
			geom.nNumVertexStreams = VSF_HWSKIN_INFO + 1;
		}
	}
