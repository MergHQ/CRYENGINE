...
#define IFT(x) { HRESULT __hr = (x); if (DXC_FAILED(__hr)) return E_FAIL; }
...
HRESULT WINAPI BridgeD3DCompile(LPCVOID pSrcData, SIZE_T SrcDataSize,
	...
	*ppCode = nullptr;
	...
	IFR(library->CreateBlobWithEncodingFromPinned((LPBYTE)pSrcData, (UINT32)SrcDataSize,
		CP_ACP, &source));
...
HRESULT WINAPI BridgeD3DReflect(
	...
	*ppReflector = nullptr;
	...
	IFR(CreateLibrary(&library));
	IFR(library->CreateBlobWithEncodingOnHeapCopy((LPBYTE)pSrcData, (UINT32)SrcDataSize,
		CP_ACP, &source));
