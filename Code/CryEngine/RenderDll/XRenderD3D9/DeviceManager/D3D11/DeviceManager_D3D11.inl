  ...
  , D3DBuffer** ppBuff)
{
...
#endif
	{
		...
		default:
			// It's perfectly valid to allocate a DYNAMIC/STAGING resource for non-bind purposes (backup, staging, etc.)
			break;
		}
	}
#if !defined(OPENGL)
...
uint8* CDeviceManager::Map(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode)
{
...
void CDeviceManager::Unmap(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode)
{
...
void CDeviceManager::UploadContents(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode, const void* pInDataCPU, void* pOutDataGPU, UINT numDataBlocks)
{
	if (!bDirectAccess)
	{
		const uint8* pInData = reinterpret_cast<const uint8*>(pInDataCPU);
		uint8* pOutData = CDeviceManager::Map(buffer, subresource, offset, 0, mode) + offset;

		StreamBufferData(pOutData, pInData, size);

		CDeviceManager::Unmap(buffer, subresource, offset, size, mode);
	}
	else
	{
		// Transfer sub-set of GPU resource to CPU, also allows graphics debugger and multi-gpu broadcaster to do the right thing
		CDeviceManager::MarkReadRange(buffer, offset, 0, D3D11_MAP_WRITE);

		StreamBufferData(pOutDataGPU, pInDataCPU, size);

		CDeviceManager::MarkWriteRange(buffer, offset, size, D3D11_MAP_WRITE);
	}
}

template<const bool bDirectAccess>
void CDeviceManager::DownloadContents(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode, void* pOutDataCPU, const void* pInDataGPU, UINT numDataBlocks)
{
	if (!bDirectAccess)
	{
		uint8* pOutData = reinterpret_cast<uint8*>(pOutDataCPU);
		const uint8* pInData = CDeviceManager::Map(buffer, subresource, offset, size, mode) + offset;

		FetchBufferData(pOutData, pInData, size);

		CDeviceManager::Unmap(buffer, subresource, offset, 0, mode);
	}
	else
	{
		// Transfer sub-set of GPU resource to CPU, also allows graphics debugger and multi-gpu broadcaster to do the right thing
		CDeviceManager::MarkReadRange(buffer, offset, size, D3D11_MAP_READ);

		FetchBufferData(pOutDataCPU, pInDataGPU, size);

		CDeviceManager::MarkWriteRange(buffer, offset, 0, D3D11_MAP_READ);
	}
}

// Explicit instantiation
template
void CDeviceManager::UploadContents<true>(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode, const void* pInDataCPU, void* pOutDataGPU, UINT numDataBlocks);
template
void CDeviceManager::UploadContents<false>(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode, const void* pInDataCPU, void* pOutDataGPU, UINT numDataBlocks);

template
void CDeviceManager::DownloadContents<true>(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode, void* pOutDataCPU, const void* pInDataGPU, UINT numDataBlocks);
template
void CDeviceManager::DownloadContents<false>(D3DBuffer* buffer, uint32 subresource, size_t offset, size_t size, D3D11_MAP mode, void* pOutDataCPU, const void* pInDataGPU, UINT numDataBlocks);
