...
{
	CEnvRegistrationScope scope = registrar.Scope(GetTypeInfo<float>().GetGUID());
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&Add, "d62aede8-ed2c-47d9-b7f3-6de5ed0ef51f"_schematyc_guid, "Add");
		pFunction->SetAuthor("Dominique Mader");
...
static void RegisterFunctions(IEnvRegistrar& registrar)
{
	CEnvRegistrationScope scope = registrar.Scope(GetTypeInfo<Vec2>().GetGUID());
	{
...
static void RegisterFunctions(IEnvRegistrar& registrar)
{
	CEnvRegistrationScope scope = registrar.Scope(GetTypeInfo<Vec3>().GetGUID());
	{
