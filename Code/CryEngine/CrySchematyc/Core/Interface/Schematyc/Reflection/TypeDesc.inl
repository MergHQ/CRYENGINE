...
inline bool SerializeClass(Serialization::IArchive& archive, const CClassDesc& typeDesc, void* pValue, const char* szName, const char* szLabel)
{
	...
	public:
		...
		inline CSerializer(const CClassDesc& typeDesc, void* pValue)
			: m_typeDesc(typeDesc)
			, m_pValue(pValue)
		{}
	...
	return archive(CSerializer(typeDesc, pValue), szName, szLabel);
}
...
inline void CCommonTypeDesc::SetGUID(const SGUID& guid)
{
	m_guid = guid;
}

inline const SGUID& CCommonTypeDesc::GetGUID() const
{
	return m_guid;
}
...
{
	SCHEMATYC_CORE_ASSERT(m_pElementTypeDesc);
...
inline const ClassBaseDescs& CClassDesc::GetBases() const
{
	return m_bases;
}
...
inline const ClassMemberDescs& CClassDesc::GetMembers() const
{
	return m_members;
}
...
inline bool CClassDesc::AddBase(const CCommonTypeDesc& typeDesc, ptrdiff_t offset)
{
	...
	SCHEMATYC_CORE_ASSERT(!bDuplicateTypeDesc);
...
inline bool CClassDesc::AddMember(const CCommonTypeDesc& typeDesc, ptrdiff_t offset, uint32 id, const char* szName, const char* szLabel, const char* szDescription, Utils::IDefaultValuePtr&& pDefaultValue)
{
	const bool bDuplicateOffset = FindMemberByOffset(offset) != nullptr;
	SCHEMATYC_CORE_ASSERT(!bDuplicateOffset);
	if (bDuplicateOffset)
	{
		return false;
	}

	const bool bDuplicateId = FindMemberByOffset(id) != nullptr;
	SCHEMATYC_CORE_ASSERT(!bDuplicateId);
	if (bDuplicateId)
	{
		return false;
	}
	...
	SCHEMATYC_CORE_ASSERT(!bDuplicateName);
	if (bDuplicateName)
	{
		return false;
	}
