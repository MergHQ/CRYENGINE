	...
	, m_actionInputID(0)
{
	...
	gEnv->pAudioSystem->AddRequestListener(&CommunicationHandler::TriggerFinishedCallback, this, eSystemEvent_TriggerFinished);
}
	...
	if (pEntity)
	{
		...
		if (pIEntityAudioComponent)
		{
			...
			if (playCommunicationControlId != InvalidControlId)
			{
				...
				SRequestUserData const userData(eRequestFlags_PriorityNormal | eRequestFlags_SyncFinishedCallback, this, reinterpret_cast<void*>(static_cast<UINT_PTR>(m_entityId)), this);
