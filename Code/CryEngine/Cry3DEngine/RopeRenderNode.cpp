...
void CRopeRenderNode::Render(const SRendParams& rParams, const SRenderingPassInfo& passInfo)
{
	...
	int lod = min((int)CRY_ARRAY_COUNT(m_pRenderMeshLods) - 1, (int)(rParams.fDistance * m_lodRatio * gEnv->p3DEngine->GetFrameLodInfo().fTargetSize));
	for(; lod > 0 && !m_pRenderMeshLods[lod - 1]; lod--)
		;
	(lod ? m_pRenderMeshLods[lod - 1] : m_pRenderMesh)->Render(pObj, passInfo);
}
...
//////////////////////////////////////////////////////////////////////////
void CRopeRenderNode::CreateRenderMesh(int lod)
{
	// make new RenderMesh
	//////////////////////////////////////////////////////////////////////////
	_smart_ptr<IRenderMesh> pRenderMesh = GetRenderer()->CreateRenderMeshInitialized(
	...
	pRenderMesh->SetChunk(0, chunk);

	(lod ? m_pRenderMeshLods[lod - 1] : m_pRenderMesh) = pRenderMesh;
}
...
void CRopeRenderNode::UpdateRenderMesh()
{
	...
	int nNewVertexCount = m_params.nNumSides * (nLengthSamples - 1) * 3 - 1, nLods = 1;
	...
	if (!m_params.segmentObj.empty())
	{
		...
		for(nLods = 0; m_segObj->GetLodObject(nLods); nLods++)
			;
	}
	for(int i = CRY_ARRAY_COUNT(m_pRenderMeshLods) - 1; i > nLods - 2; m_pRenderMeshLods[i--] = nullptr)
		;
	...
	if (m_pRenderMesh->GetVerticesCount() != nNewVertexCount || m_paramsChanged || !useBones)
	{
		...
		for(int lod = 0; lod < nLods; lod++)
		{
			...
			if (lod)
			{
				if (!m_pRenderMeshLods[lod - 1])
					CreateRenderMesh(lod);
				pRenderMesh = m_pRenderMeshLods[lod - 1];
				nNewVertexCount = m_segObj->GetLodObject(lod)->GetRenderMesh()->GetVerticesCount() * m_params.nNumSegments;
			}
			pRenderMesh->LockForThreadAccess();
			pRenderMesh->UpdateVertices(NULL, nNewVertexCount, 0, VSF_GENERAL, 0u);

			strided_pointer<Vec3> vtx;
			vtx.data = (Vec3*)pRenderMesh->GetPosPtr(vtx.iStride, FSL_VIDEO_CREATE);
			strided_pointer<Vec2> tex;
			tex.data = (Vec2*)pRenderMesh->GetUVPtr(tex.iStride, FSL_VIDEO_CREATE);
			SMeshBoneMapping_uint16* skin = nullptr;

			if (useBones)
			{
				strided_pointer<SMeshQTangents> qtng;
				qtng.data = (SMeshQTangents*)pRenderMesh->GetQTangentPtr(qtng.iStride, FSL_VIDEO_CREATE);
				...
				if (!m_params.segmentObj.empty())
				{
					...
					IRenderMesh* pSegMesh = m_segObj->GetLodObject(lod)->GetRenderMesh();
					...
					m_lodRatio = 1 / (m_segObj->GetLodDistance() * trans.s);
					...
					if (clrSeg.data = (byte*)pSegMesh->GetColorPtr(clrSeg.iStride, FSL_READ))
						clr.data = (byte*)pRenderMesh->GetColorPtr(clr.iStride, FSL_VIDEO_CREATE);
					...
					pRenderMesh->UpdateIndices(&m_tmpIdx[0].x, ntris * 3 * m_params.nNumSegments, 0, 0u);
				}
				else
				{
					...
					pRenderMesh->UpdateIndices(&idx[0].x, ivtx * 3, 0, 0u);
				}
				pRenderMesh->UnlockStream(VSF_QTANGENTS);
			}
			else
			{
				strided_pointer<SPipTangents> tng;
				tng.data = (SPipTangents*)pRenderMesh->GetTangentPtr(tng.iStride, FSL_VIDEO_CREATE);
				...
				pRenderMesh->UpdateIndices(tubeSurf.m_pIndices, tubeSurf.iNumIndices, 0, 0u);
				m_bones.resize(0);
				pRenderMesh->SetSkinned(false);
			...
			// Update chunk params.
			CRenderChunk* pChunk = &pRenderMesh->GetChunks()[0];
			if (m_pMaterial)
				pChunk->m_nMatFlags = m_pMaterial->GetFlags();
			pChunk->nNumIndices = pRenderMesh->GetIndicesCount();
			pChunk->nNumVerts = nNewVertexCount;
			pRenderMesh->SetChunk(0, *pChunk);

			if (useBones)
			{
				pRenderMesh->CreateChunksSkinned();
				pChunk = &pRenderMesh->GetChunksSkinned()[0];
				pChunk->m_bUsesBones = true;
				pChunk->pRE->mfUpdateFlags(FCEF_SKINNED);
				static CMesh mesh;
				pRenderMesh->SetSkinningDataCharacter(mesh, 0, skin, nullptr);
				pRenderMesh->SetSkinned(true);
			}
