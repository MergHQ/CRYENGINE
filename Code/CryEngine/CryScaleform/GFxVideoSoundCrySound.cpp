...
class GFxVideoCrySoundSystemImpl : public GNewOverrideBase<Stat_Video_Mem>
{
public:
...
private:
	...
	public:
		...
		{
			++m_numAllocs;
			return SF_HEAP_ALLOC(m_pHeap, size, Stat_Video_Mem);
		}

		void Free(void* p)
		{
			SF_HEAP_FREE(m_pHeap, p);
			--m_numAllocs;
		}
...
{
public:
	virtual bool      CreateOutput(UInt32 numChannels, UInt32 sampleRate) override;
	virtual void      DestroyOutput() override;

	virtual PCMFormat GetPCMFormat() const override { return PCM_SInt16; }
	virtual void      Start(PCMStream* pStream) override;
	virtual void      Stop() override;
	virtual Status    GetStatus() const override { return m_status; }

	virtual void      Pause(bool pause) override;
	virtual void      GetTime(UInt64* pCount, UInt64* pUnit) const override;

public:
	GFxVideoCrySound(GMemoryHeap* pHeap, GFxVideoCrySoundSystemImpl* pSoundSystem);
...
void GFxVideoCrySoundSystemImpl::AttachSound(GFxVideoCrySound* pSound)
{
	...
	if (!m_pUpdateThread)
	{
		...
		m_pUpdateThread = *SF_HEAP_NEW(m_pHeap) GThread(params);
...
void GFxVideoCrySound::ReleaseBuffers()
{
	for (size_t ch = 0; ch < MaxChannels; ++ch)
	{
		...
		{
			SF_HEAP_FREE(m_pHeap, m_pChannelBuffer[ch]);
	...
	{
		SF_HEAP_FREE(m_pHeap, m_pInterleavedBuffer);
		m_pInterleavedBuffer = 0;
	}
...
void GFxVideoCrySound::UpdateVolume(bool muteMainTrack)
{
	...
	const float newVolume = clamp_tpl(s_sys_flash_video_soundvolume, 0.0f, 1.0f);
...
bool GFxVideoCrySound::CreateOutput(UInt32 numChannels, UInt32 sampleRate)
{
	...
	m_pInterleavedBuffer = (unsigned char*) SF_HEAP_ALLOC(m_pHeap, m_bufferSize, Stat_Video_Mem);
	if (!m_pInterleavedBuffer)
	{
		ReleaseBuffers();
		return false;
	...
	for (size_t ch = 0; ch < m_numChannels; ++ch)
	{
		...
		m_pChannelBuffer[ch] = (SoundSampleType*) SF_HEAP_ALLOC(m_pHeap, m_bufferSamples * sizeof(SoundSampleType), Stat_Video_Mem);
		if (!m_pChannelBuffer[ch])
		{
			ReleaseBuffers();
			return false;
...
GFxVideoCrySoundSystem::GFxVideoCrySoundSystem(GMemoryHeap* pHeap)
	...
	, m_pImpl(SF_HEAP_NEW(GetHeap()) GFxVideoCrySoundSystemImpl(GetHeap()))
{
}
...
GFxVideoSound* GFxVideoCrySoundSystem::Create()
{
	VideoSoundTimer::InitFrequency();
	return m_pImpl ? (SF_HEAP_NEW(GetHeap()) GFxVideoCrySound(GetHeap(), m_pImpl /*, GFxVideoPlayer::MainTrack*/)) : 0;
}
