...
// flash player implementation via Scaleform's GFx
	...
	#pragma warning(push)
	#pragma warning(disable : 4266) // function is hidden
...
public:
		...
		, m_capture(capture)
	{
		...
		IF (m_capture, 0)
		{
			...
			m_pRenderer->GetStats(&stats, false);
...
{
public:
	CSyncSection(GFxMovieView* pMovieView, CryCriticalSectionPtr pSection)
		: m_pMovieView(pMovieView), m_pSection(pSection)
	{
		m_pSection->Lock();
		if (m_pMovieView)
		{
			m_pMovieView->SetCaptureThread((Scaleform::ThreadId)CryGetCurrentThreadId());
		}
	}
...
private:
	CryCriticalSectionPtr m_pSection;
	GFxMovieView*         m_pMovieView;
};

	#define SYNC_THREADS         CryAutoCriticalSection theLock(*m_lock.get());
	#if defined(_DEBUG)
		#define SYNC_FLASH_THREADS CSyncSection theLock(m_pMovieView, m_lock);
		#define SYNC_VALUE_THREADS CSyncSection theLock(m_value.GetMovie(), m_lock);
	#else
		#define SYNC_FLASH_THREADS CryAutoCriticalSection theLock(*m_lock.get());
		#define SYNC_VALUE_THREADS CryAutoCriticalSection theLock(*m_lock.get());
	#endif
...
public:
	...
	SF_MEMORY_REDEFINE_NEW(CFlashVariableObject, Stat_Default_Mem)
...
bool CFlashVariableObject::SetMember(const char* pMemberName, const IFlashVariableObject* pVarObj)
{
	...
	bool res(false);
	if (m_value.IsObject())
		res = m_value.SetMember(pMemberName, pVarObj ? Scaleform4::GetGFxValue(pVarObj) : GFxValue());
	return res;
}
...
bool CFlashVariableObject::SetElement(unsigned int idx, const IFlashVariableObject* pVarObj)
{
	...
	bool res(false);
	if (m_value.IsArray())
		res = m_value.SetElement(idx, pVarObj ? Scaleform4::GetGFxValue(pVarObj) : GFxValue());
	return res;
}
...
bool CFlashVariableObject::PushBack(const IFlashVariableObject* pVarObj)
{
	...
	bool res(false);
	if (m_value.IsArray())
		res = m_value.PushBack(pVarObj ? Scaleform4::GetGFxValue(pVarObj) : GFxValue());
	return res;
}
...
bool CFlashVariableObject::SetDisplayInfo(const SFlashDisplayInfo& info)
{
	...
	if (m_value.IsDisplayObject())
	{
		unsigned short varsSet =
			...
			(info.IsFlagSet(SFlashDisplayInfo::FDIF_Fov) ? GFxValue::DisplayInfo::V_FOV : 0) |
			(info.IsFlagSet(SFlashDisplayInfo::FDIF_EdgeAA) ? GFxValue::DisplayInfo::V_edgeaaMode : 0);
		...
		di.Initialize(varsSet,
		              ...
		              info.IsFlagSet(SFlashDisplayInfo::FDIF_Fov) ? info.GetFov() : 0,
		              nullptr,
		              nullptr,
		              info.IsFlagSet(SFlashDisplayInfo::FDIF_EdgeAA) ? (EdgeAAMode)info.GetEdgeAA() : EdgeAA_Inherit);
	...
	if (m_value.IsDisplayObject())
	{
		...
		if (res)
		{
			...
			info = SFlashDisplayInfo(
				...
				di.IsFlagSet(GFxValue::DisplayInfo::V_FOV) ? (float) di.GetFOV() : 0,
				di.IsFlagSet(GFxValue::DisplayInfo::V_edgeaaMode) ? (SFlashDisplayInfo::EdgeAAMode)di.GetEdgeAAMode() : SFlashDisplayInfo::FDIEAAM_Inherit,
				varsSet);
		}
	}
	return res;
...
bool CFlashVariableObject::SetVisible(bool visible)
{
	...
	{
		GFxValue::DisplayInfo di;
		di.Initialize(GFxValue::DisplayInfo::V_visible, 0, 0, 0, 0, 0, 0, visible, 0, 0, 0, 0, 0, nullptr, nullptr, EdgeAA_Inherit);
...
bool CFlashVariableObject::SetColorTransform(const SFlashCxform& cx)
{
	...
	if (m_value.IsDisplayObject())
	{
		...
		cxform.M[0][0] = cx.mul.r;
		cxform.M[0][1] = cx.mul.g;
		cxform.M[0][2] = cx.mul.b;
		cxform.M[0][3] = cx.mul.a;
		cxform.M[1][0] = static_cast<float>(cx.add.r);
		cxform.M[1][1] = static_cast<float>(cx.add.g);
		cxform.M[1][2] = static_cast<float>(cx.add.b);
		cxform.M[1][3] = static_cast<float>(cx.add.a);
...
bool CFlashVariableObject::GetColorTransform(SFlashCxform& cx) const
{
	...
	if (m_value.IsDisplayObject())
	{
		...
		if (res)
		{
			cx.mul.r = cxform.M[0][0];
			cx.mul.g = cxform.M[0][1];
			cx.mul.b = cxform.M[0][2];
			cx.mul.a = cxform.M[0][3];
			cx.add.r = static_cast<uint8>(cxform.M[1][0]);
			cx.add.g = static_cast<uint8>(cxform.M[1][1]);
			cx.add.b = static_cast<uint8>(cxform.M[1][2]);
			cx.add.a = static_cast<uint8>(cxform.M[1][3]);
		}
	}
	return res;
}
...
bool CFlashVariableObject::AttachMovie(IFlashVariableObject*& pVarObjMC, const char* pSymbolName, const char* pInstanceName, int depth, const IFlashVariableObject* pInitObj)
{
	...
	{
		GFxValue retVal;
		if (m_value.AttachMovie(&retVal, pSymbolName, pInstanceName, depth))
		{
			//assert(retVal.IsObject());
			pVarObjMC = new CFlashVariableObject(retVal, m_refFilePath, m_lock);
		}
...
public:
	#if defined(USE_GFX_POOL)
	SF_MEMORY_REDEFINE_NEW(CFlashPlayerBootStrapper, Stat_Default_Mem)
	void* operator new(size_t, void* p) throw() { return p; }
	void* operator new[](size_t, void* p) throw() { return p; }
	void  operator delete(void*, void*) throw()   {}
	void  operator delete[](void*, void*) throw() {}
	...
	#endif
{
	...
	m_pRenderCache = *SF_HEAP_AUTO_NEW(m_pRenderer.GetPtr()) CSFRenderCache();
	#if defined(ENABLE_FLASH_INFO)
	m_node.m_pHandle = this;
	GetList().Link(m_node);
	#endif
...
{
	if (!pMovieDef || !pFilePath || !m_pRenderer)
		return false;
	...
	ThreadCommandQueue* pCommandQueue = m_pRenderer->GetThreadCommandQueue();
	// create movie view
	GPtr<GFxMovieView> pMovieView = *pMovieDef->CreateInstance((options & INIT_FIRST_FRAME) != 0, m_memArenaID, nullptr, pCommandQueue);
	if (!pMovieView)
		return false;
	...
	*m_filePath = pFilePath;
	...
	m_allowEdgeAA = (options & RENDER_EDGE_AA) != 0;
	UpdateRenderFlags();
...
void CFlashPlayer::SetViewport(int x0, int y0, int width, int height, float aspectRatio)
{
	...
	if (m_pMovieView)
	{
		...
		viewport.BufferWidth = x0 + width;
		viewport.BufferHeight = y0 + height;
...
Scaleform::Render::RenderTarget* CFlashPlayer::CreateRenderTarget(CTexture* pTexture)
{
	CRenderOutputPtr pRenderOutput;

	if (pTexture)
	{
		GViewport sfVP;
		m_pMovieView->GetViewport(&sfVP);
		SRenderViewport cryVP(sfVP.Left, sfVP.Top, sfVP.Width, sfVP.Height);

		pRenderOutput = std::make_shared<CRenderOutput>(pTexture, FRT_CLEAR_COLOR, Clr_Transparent, 0.0f);
		pRenderOutput->SetViewport(cryVP);
	}

	{
		CD3D9Renderer* pRenderer = (CD3D9Renderer*)gEnv->pRenderer;
		if (!pRenderOutput)
		{
			if (auto* pDC = pRenderer->GetActiveDisplayContext())
			{
				pRenderOutput = pDC->GetRenderOutput();
			}
		}

		if (!pRenderOutput)
		{
			if (auto* pDC = pRenderer->GetBaseDisplayContext())
			{
				pRenderOutput = pDC->GetRenderOutput();
			}
		}
	...
	CRY_ASSERT(pRenderOutput);
	if (!pRenderOutput)
	{
		return nullptr;
	}

	CSFRenderer* pSFRenderer = ((CSFRenderer*)m_pRenderer.GetPtr());
	CTexture* pRenderTarget = nullptr;
	CTexture* pDepthTarget = nullptr;

	if (auto* pDC = pRenderOutput->GetDisplayContext())
	{
		pRenderTarget = pDC->GetCurrentColorOutput();
		pDepthTarget = pDC->GetCurrentDepthOutput();
	}
	else
	{
		pRenderTarget = pRenderOutput->GetColorTarget();
		pDepthTarget = pRenderOutput->GetDepthTarget();
	}

	if (m_clearFlags)
	{
		CClearSurfacePass clearPass;
		if (pRenderTarget && (m_clearFlags & FRT_CLEAR_COLOR) && !(m_clearFlags & FRT_CLEAR_FOGCOLOR))
		{
			clearPass.Execute(pRenderTarget, Clr_Transparent);
		}
		if (pDepthTarget && (m_clearFlags & FRT_CLEAR_STENCIL))
		{
			clearPass.Execute(pDepthTarget, CLEAR_STENCIL, Clr_Unused.r, Val_Stencil);
		}
	}

	return pSFRenderer->CreateRenderTarget(pRenderTarget, pDepthTarget);
}
...
void CFlashPlayer::RenderToTexture(ITexture* pTexture)
{
	...
	GPtr<Scaleform::Render::RenderTarget> pSFTarget = *CreateRenderTarget(static_cast<CTexture*>(pTexture));
	CSFRenderer* pSFRenderer = static_cast<CSFRenderer*>(m_pRenderer.GetPtr());

	if (pSFTarget)
	{
		Scaleform::Render::StereoParams stereoParams;
		IStereoRenderer* pStereoRenderer = gEnv->pRenderer->GetIStereoRenderer();
		if (pStereoRenderer)
		{
			// TODO: convert pStereoRenderer properties to stereoParams here
			// Note: look at the scaleform code Render_MatrixState.cpp function StereoImplBase::GetStereoProj to see how the params are used -
			// - all params are reduced in the end into 1 variable that determines the distance between the eyes (see line 261 in Render_MatrixState.cpp)
			// OR isntead of filling the stereo params, it is possible to overridde the StereoImplBase::GetStereoProj to provide own left/right eye matrix -
			// - see MatrixState::updateStereoProjection for its usage and MatrixState::recalculateUVPOC() to see the whole matrix pipeline
			pSFRenderer->SetStereoParams(stereoParams);
		}

		pSFRenderer->SetRenderStatesCache(m_pRenderCache);
		pSFRenderer->BeginFrame();
		pSFRenderer->SetRenderTarget(pSFTarget);

		Scaleform::GFx::MovieDisplayHandle hMovieDisplay = m_pMovieView->GetDisplayHandle();
		if (hMovieDisplay.NextCapture(pSFRenderer->GetContextNotify()))
		{
			pSFRenderer->BeginScene();
			pSFRenderer->Display(hMovieDisplay);
			pSFRenderer->EndScene();
		}

		pSFRenderer->SetRenderTarget(nullptr);
		pSFRenderer->EndFrame();
		pSFRenderer->SetRenderStatesCache(nullptr);
	}
...

	{
		...
		//#endif
		{
...

			{
				...
				m_pRenderer->SetStereoDisplay(static_cast<Scaleform::Render::StereoDisplay>(ft));
...
{
	// This is most likely just a legacy function from the times when Scaleform3
	// was executed and also rendered from the main thread.
	assert(0 && "CFlashPlayer::RenderPlaybackLocklessCallback() -- Not implemented!");
}
...
{
	//SYNC_FLASH_THREADS; //don't sync (should hardly produce read/write conflicts and thus rendering glitches, if ever)
...
{
	//SYNC_FLASH_THREADS; //don't sync (handler needs to reside in same thread as flash player instance)
...
void CFlashPlayer::SendKeyEvent(const SFlashKeyEvent& keyEvent)
{
	...
	if (m_pMovieView)
	{
		...
		event.Modifiers.SetShiftPressed((keyEvent.m_specialKeyState & SFlashKeyEvent::eShiftPressed) != 0);
		event.Modifiers.SetCtrlPressed((keyEvent.m_specialKeyState & SFlashKeyEvent::eCtrlPressed) != 0);
		event.Modifiers.SetAltPressed((keyEvent.m_specialKeyState & SFlashKeyEvent::eAltPressed) != 0);
		event.Modifiers.SetCapsToggled((keyEvent.m_specialKeyState & SFlashKeyEvent::eCapsToggled) != 0);
		event.Modifiers.SetNumToggled((keyEvent.m_specialKeyState & SFlashKeyEvent::eNumToggled) != 0);
		event.Modifiers.SetScrollToggled((keyEvent.m_specialKeyState & SFlashKeyEvent::eScrollToggled) != 0);
...
bool CFlashPlayer::SetOverrideTexture(const char* pResourceName, ITexture* pTexture, bool resize)
{
	...
	if (pTexture)
	{
		...
		if (pimageRes)
		{
			Scaleform::Render::ImageSize size(resize ? pimageRes->GetWidth() : 0, resize ? pimageRes->GetHeight() : 0);
			GPtr<Scaleform::Render::Texture> pSFtexture = *static_cast<CSFTextureManager*>(m_pRenderer->GetTextureManager())->CreateTexture(static_cast<CTexture*>(pTexture), size);
			assert(pSFtexture);
			if (pSFtexture)
			{
				Image* pImage = SF_HEAP_AUTO_NEW(m_pRenderer) CryTextureImage(pSFtexture, 0);
				pimageRes->SetImage(pImage);
			}
			return true;
		}
	}
...
size_t CFlashPlayer::GetMetadata(char* pBuff, unsigned int buffSize) const
{
	...
	//SYNC_FLASH_THREADS; // should be safe as meta data for the root is only assigned during Load()
...
{
	//SYNC_FLASH_THREADS; // should be safe as m_filePath is only modified during Load()
...
{
	return m_allowEdgeAA && (ms_sys_flash_edgeaa != 0);
}

void CFlashPlayer::UpdateRenderFlags()
{
	if (m_pMovieView && (IsEdgeAaAllowed() != (m_pMovieView->GetEdgeAAMode() == EdgeAA_On)))
		m_pMovieView->SetEdgeAAMode(IsEdgeAaAllowed() ? EdgeAA_On : EdgeAA_Off);
}
...
void CFlashPlayer::RenderFlashInfo()
{
	...
	std::vector<ITexture*> renderTargets;

	if (ms_sys_flash_info)
	{
		IF (ms_sys_flash_info == 3, 0)
		{
			unsigned glyphTextureCount = pFlashRenderer->GetGlyphCache()->GetNumTextures();
			for (unsigned i = 0; i < glyphTextureCount; ++i)
			{
				Image* pImage = pFlashRenderer->GetGlyphCache()->GetImage(i);
				CSFTexture* pTexture = static_cast<CSFTexture*>(pImage->GetTexture(pFlashRenderer->GetTextureManager()));
				if (pTexture && pTexture->GetCryTexture())
				{
					//pRenderer->SetState(GS_NODEPTHTEST);
					IRenderAuxImage::Draw2dImage((float)(i * 450), 0, 450, 450, -1, 0.0f, 1.0f, 1.0f, 0.0f);
					//pRenderer->SetState(GS_NODEPTHTEST | GS_BLSRC_SRCALPHA | GS_BLDST_ONEMINUSSRCALPHA);
					//pRenderer->Draw2dImageStretchMode(true);
					IRenderAuxImage::Draw2dImage((float)(i * 450), 0, 450, 450, pTexture->GetCryTexture()->GetTextureID(), 0.0f, 1.0f, 1.0f, 0.0f);
		...
		Scaleform::Render::HAL::Stats stats;
		if (pFlashRenderer)
			pFlashRenderer->GetStats(&stats, true);

		DrawText(xAdj + 10.0f, yAdj + 10.0f, color, "Scaleform version: %d.%d.%d", GFX_MAJOR_VERSION, GFX_MINOR_VERSION, GFX_BUILD_VERSION);
		DrawText(xAdj + 10.0f, yAdj + 24.0f, color, "#Tris      : %5d", stats.Triangles);
		DrawText(xAdj + 10.0f, yAdj + 36.0f, color, "#Masks     : %5d", stats.Masks);
		DrawText(xAdj + 10.0f, yAdj + 48.0f, color, "#DrawCalls : %5d", stats.Primitives);
		DrawText(xAdj + 10.0f, yAdj + 64.0f, color, "#Filters   : %5d", stats.Filters);

		// display memory statistics
		if (pGFxHeap)
		{
			size_t memMeshCache = 0;
			size_t memFontCache = 0;
			size_t memTextureCache = 0;
			int32 textureCount = 0;
			int32 textureCreations = 0;
			static int32 s_lastNumTexturesTotal = 0;

			if (pFlashRenderer)
			{
				Scaleform::Render::MeshCache::Stats meshStats;
				pFlashRenderer->GetMeshCache().GetStats(&meshStats);
				for (int i = 0; i < Scaleform::Render::MeshCache::MeshBuffer_GpuMem; ++i)
				{
					memMeshCache += meshStats.GetTotal(i);
				}

				if (pFlashRenderer->GetGlyphCache())
				{
					memFontCache = pFlashRenderer->GetGlyphCache()->GetBytes();
				}

				if (pFlashRenderer->GetTextureManager())
				{
					((CSFTextureManager*)pFlashRenderer->GetTextureManager())->GetTexturesStats(memTextureCache, textureCount, textureCreations, renderTargets);
				}
			}
			...
			DrawText(xAdj + 10.0f, yAdj + 170.0f, color, "TexMemory Used       : %6dk", InKB(memTextureCache));
			...
			bool texturesAtRuntime = textureCreations > s_lastNumTexturesTotal;
			if (texturesAtRuntime)
				s_lastNumTexturesTotal = textureCreations;
			DrawText(xAdj + 300.0f, yAdj + 170.0f, texturesAtRuntime ? colorListSelected : color, "#Tex: %d / #TexCreations: %d%s",
			         textureCount, s_lastNumTexturesTotal, texturesAtRuntime ? " !Textures created at runtime!" : "");
		...
		// Render target info
		int renderTargetCount = renderTargets.size();
		if (renderTargetCount > 0)
		{
			ypos += 8.f;

			DrawText(xAdj + 10.0f, yAdj + ypos, color, "Render targets (%d):", renderTargetCount);
			ypos += 12.f;

			for (int i = 0; i < renderTargetCount; i++)
			{
				ITexture* pTex = renderTargets[i];
