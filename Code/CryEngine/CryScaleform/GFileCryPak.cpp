...
bool GFileCryPak::IsValid() const
{
	return IsValidInternal();
}

bool GFileCryPak::IsWritable() const
{
...
bool GFileCryStream::IsValid() const
{
	return IsValidInternal();
}

bool GFileCryStream::IsWritable() const
{
	// writable files are not supported!
	return false;
}
	...
	, m_errCode(EC_Undefined)
{
	...
	if (pPath && *pPath)
	{
		...
		m_pData = m_dataSize ? (unsigned char*)SF_ALLOC(m_dataSize, Stat_Default_Mem) : 0;
...
bool GFileInMemoryCryStream::IsValid() const
{
	return m_pMemFile->IsValid();
}

bool GFileInMemoryCryStream::IsWritable() const
{
	// writable files are not supported!
