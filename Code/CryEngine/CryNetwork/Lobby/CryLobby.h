...
public:
	...
	virtual CLobbyCVars&               GetCVars() { return m_cvars; }

	virtual void                       Tick(bool flush);
	...
	virtual ICryOnlineStorage*         GetOnlineStorage()                        { return m_services[m_service] ? m_services[m_service]->GetOnlineStorage() : NULL; }
...
#endif
	virtual void                 AddHostMigrationEventListener(IHostMigrationEventListener* pListener, const char* pWho, uint32 priority);
	...
	virtual void        SetCryEngineLoadHint(const char* pLevelName, const char* pGameRules) { cry_strcpy(m_lvlName, pLevelName); cry_strcpy(m_rulName, pGameRules); }
