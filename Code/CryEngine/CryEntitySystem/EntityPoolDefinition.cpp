...
void CEntityPoolDefinition::DebugDraw(IRenderer* pRenderer, float& fColumnX, float& fColumnY) const
{
	...
	pRenderer->Draw2dLabel(fColumnX + 15.0f, fColumnY, 1.0f, m_bForcedBookmarked ? colRed : colWhite, false, "%s", sDebugName.c_str());
	fColumnY += 12.0f;
	...
	for (; itClass != itClassEnd; ++itClass)
	{
		...
		pRenderer->Draw2dLabel(fColumnX + 17.5f, fColumnY, 0.8f, pColor, false, "%s", pClass->GetName());
		fColumnY += 10.0f;
	}
}
