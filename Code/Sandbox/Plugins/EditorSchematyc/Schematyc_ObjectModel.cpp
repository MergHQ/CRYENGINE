...
#include "Schematyc_StateMachineItem.h"
...
CObjectModel::CObjectModel(Schematyc::IScriptView& scriptView)
	...
	, m_scriptRegistry(GetSchematycFramework().GetScriptRegistry())
	, m_scriptElement(*GetSchematycFramework().GetScriptRegistry().GetElement(scriptView.GetScopeGUID()))
{
...
CAbstractVariablesModelItem* CObjectModel::CreateVariable()
{
	...
	Schematyc::IScriptVariable* pVariableElement = m_scriptRegistry.AddVariable(uniqueName, Schematyc::SElementId(), &m_scriptElement);
	if (pVariableElement)
	{
