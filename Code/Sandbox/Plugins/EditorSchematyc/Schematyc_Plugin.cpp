...
#include <Schematyc/SerializationUtils/Schematyc_SerializationEnums.inl>
#include <Schematyc/Utils/Schematyc_GUID.h>
...
CSchematycPlugin::CSchematycPlugin()
{
	...
	GetSchematycFramework().SetGUIDGenerator(Schematyc::Delegate::Make(GenerateGUID));
	CryLogAlways("[SchematycEditor]: Fixing up script files");
	GetSchematycFramework().GetScriptRegistry().ProcessEvent(Schematyc::SScriptEvent(Schematyc::EScriptEventId::EditorFixUp));
