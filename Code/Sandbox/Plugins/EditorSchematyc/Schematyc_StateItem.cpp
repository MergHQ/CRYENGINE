...
QIcon CStateItem::s_icon = CryIcon("icons:schematyc/script_state.ico");
...
bool CStateItem::RemoveGraph(CGraphItem& functionItem)
{
	...
	GetSchematycFramework().GetScriptRegistry().RemoveElement(functionItem.GetGUID());
...
bool CStateItem::RemoveState(CStateItem& stateItem)
{
	...
	GetSchematycFramework().GetScriptRegistry().RemoveElement(stateItem.GetGUID());
