...
QIcon CStateMachineItem::s_icon = CryIcon("icons:schematyc/script_state_machine.ico");
...
CStateItem* CStateMachineItem::CreateState()
{
	...
	Schematyc::IScriptState* pStateElement = GetSchematycFramework().GetScriptRegistry().AddState(name, m_model.GetScriptElement());
	if (pStateElement)
	{
