...
struct IEditorImpl
{
	...
	virtual IImplItem* GetItem(ControlId const id) const = 0;

	//! Returns the icon corresponding to the middleware control passed as argument.
	//! \param pIImplItem - Middleware control
	//! \return A string with the path to the icon corresponding to the control type.
	virtual char const* GetTypeIcon(IImplItem const* const pIImplItem) const = 0;
	...
	//! \param assetType - An audio system control type.
	//! \param pIImplItem - A middleware control.
	//! \return A bool if the types are compatible or not.
	virtual bool IsTypeCompatible(EAssetType const assetType, IImplItem const* const pIImplItem) const = 0;
	...
	virtual EAssetType ImplTypeToAssetType(IImplItem const* const pIImplItem) const = 0;
	...
	//! \param pIImplItem - Middleware control for which to make the connection.
	// \return A pointer to the newly created connection.
	virtual ConnectionPtr CreateConnectionToControl(EAssetType const assetType, IImplItem const* const pIImplItem) = 0;
