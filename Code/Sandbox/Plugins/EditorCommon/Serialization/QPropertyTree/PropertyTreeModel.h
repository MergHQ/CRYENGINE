...
public:
	...
	CRowModel(CRowModel* pParent);

	CRowModel(const char* name, const char* label, CRowModel* pParent);

	//! Does not set the widget
	CRowModel(const char* name, const char* label, CRowModel* pParent, const yasli::TypeID& type);
	...
	bool IsWidgetSet() const { return m_pWidget != nullptr; }
	...
	const CRowModel* GetParent() const { return m_pParent; }
	CRowModel*       GetParent()       { return m_pParent; }
	...
	void Intersect(const CRowModel* pOther);
...
private:
	...
	void AddChild(CRowModel* pChild);
	...
	CPropertyTreeWidgetFactory::FactoryBase* m_pFactory;
};
