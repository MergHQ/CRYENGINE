...
#include "Serialization/PropertyTreeLegacy/PropertyOArchive.h"
#include "Serialization/PropertyTreeLegacy/PropertyIArchive.h"
#include "Serialization/PropertyTreeLegacy/PropertyTreeStyle.h"
#include "Serialization/PropertyTreeLegacy/Unicode.h"
#include "Serialization/PropertyTreeLegacy/PropertyTreeMenuHandler.h"
...
#include "Serialization/PropertyTreeLegacy/PropertyRowPointer.h"
#include "Serialization/PropertyTreeLegacy/PropertyRowContainer.h"
// ^^^
#include "Serialization/PropertyTreeLegacy/PropertyRowObject.h"
...
{
public:
	FilterEntry(QPropertyTreeLegacy* tree)
    : QLineEdit(tree)
    , tree_(tree)
	{
	}
...
void DragWindow::set(QPropertyTreeLegacy* tree, PropertyRow* row, const QRect& rowRect)
{
...
struct DrawVisitor
{
	...
	ScanResult operator()(PropertyRow* row, PropertyTreeLegacy* tree, int index)
	{
		if(row->visible(tree) && ( row->isRoot() || ((row->parent()->expanded() && !lastParent_) || row->pulledUp()))){
			...
			QDrawContext context((QPropertyTreeLegacy*)tree, &painter_);
...
class QPropertyTreeLegacy::DragController
{
public:
	DragController(QPropertyTreeLegacy* tree)
	: tree_(tree)
	, captured_(false)
	, dragging_(false)
	, before_(false)
...
QPropertyTreeLegacy::QPropertyTreeLegacy(QWidget* parent)
: QWidget(parent)
, PropertyTreeLegacy(new QUIFacade(this))
, sizeHint_(40, 40)
...
void QPropertyTreeLegacy::interruptDrag()
{
	dragController_->interrupt();
}

void QPropertyTreeLegacy::updateHeights(bool recalculateTextSize)
{
...
bool QPropertyTreeLegacy::updateScrollBar()
{
...
const QFont& QPropertyTreeLegacy::boldFont() const
{
	return boldFont_;
}

void QPropertyTreeLegacy::onScroll(int pos)
{
...
void QPropertyTreeLegacy::copyRow(PropertyRow* row)
{
...
void QPropertyTreeLegacy::pasteRow(PropertyRow* row)
{
	if(!canBePasted(row))
		return;
...
bool QPropertyTreeLegacy::canBePasted(PropertyRow* destination)
{
...
bool QPropertyTreeLegacy::canBePasted(const char* destinationType)
{
	SharedPtr<PropertyRow> source;
	if (!propertyRowFromClipboard(source, model()->constStrings()))
		return false;
...
bool QPropertyTreeLegacy::hasFocusOrInplaceHasFocus() const
{
	if (hasFocus())
		return true;
...
void QPropertyTreeLegacy::setFilterMode(bool inFilterMode)
{
...
void QPropertyTreeLegacy::startFilter(const char* filter)
{
	setFilterMode(true);
...
void QPropertyTreeLegacy::_arrangeChildren()
{
	if(widget_.get()){
...
QPoint QPropertyTreeLegacy::_toScreen(Point point) const
{
...
void QPropertyTreeLegacy::attachPropertyTree(PropertyTreeLegacy* propertyTree) 
{ 
	if(attachedPropertyTree_)
		disconnect((QPropertyTreeLegacy*)attachedPropertyTree_, SIGNAL(signalChanged()), this, SLOT(onAttachedTreeChanged()));
	PropertyTreeLegacy::attachPropertyTree(propertyTree);
	connect((QPropertyTreeLegacy*)attachedPropertyTree_, SIGNAL(signalChanged()), this, SLOT(onAttachedTreeChanged()));
}

const QMimeData* QPropertyTreeLegacy::getMimeData()
{
...
{
	const QPropertyTreeLegacy::RowFilter& filter_;

	FilterVisitor(const QPropertyTreeLegacy::RowFilter& filter) 
    : filter_(filter)
    {
    }
	...
	ScanResult operator()(PropertyRow* row, PropertyTreeLegacy* _tree)
	{
		QPropertyTreeLegacy* tree = (QPropertyTreeLegacy*)_tree;
...
void QPropertyTreeLegacy::onFilterChanged(const QString& text)
{
...
void QPropertyTreeLegacy::drawFilteredString(QPainter& p, const char* text, RowFilter::Type type, const QFont* font, const QRect& rect, const QColor& textColor, bool pathEllipsis, bool center) const
{
...
void QPropertyTreeLegacy::_drawRowValue(QPainter& p, const char* text, const QFont* font, const QRect& rect, const QColor& textColor, bool pathEllipsis, bool center) const
{
	drawFilteredString(p, text, RowFilter::VALUE, font, rect, textColor, pathEllipsis, center);
}

QSize QPropertyTreeLegacy::sizeHint() const
{
	if (sizeToContent_)
		return minimumSize();
	else
...
bool QPropertyTreeLegacy::event(QEvent* ev)
{
...
void QPropertyTreeLegacy::paintEvent(QPaintEvent* ev)
{
	QElapsedTimer timer;
	timer.start();
...
QPoint QPropertyTreeLegacy::pointToRootSpace(const QPoint& point) const
{
	return toQPoint(PropertyTreeLegacy::pointToRootSpace(fromQPoint(point)));
}

QPoint QPropertyTreeLegacy::pointFromRootSpace(const QPoint& point) const
{
	return toQPoint(PropertyTreeLegacy::pointFromRootSpace(fromQPoint(point)));
}

void QPropertyTreeLegacy::moveEvent(QMoveEvent* ev)
{
	QWidget::moveEvent(ev);
}

void QPropertyTreeLegacy::resizeEvent(QResizeEvent* ev)
{
...
void QPropertyTreeLegacy::mousePressEvent(QMouseEvent* ev)
{
...
void QPropertyTreeLegacy::mouseReleaseEvent(QMouseEvent* ev)
{
...
void QPropertyTreeLegacy::focusInEvent(QFocusEvent* ev)
{
	QWidget::focusInEvent(ev);
	_cancelWidget();
}

void QPropertyTreeLegacy::defocusInplaceEditor()
{
	if (hasFocusOrInplaceHasFocus())
		setFocus();
}

void QPropertyTreeLegacy::keyPressEvent(QKeyEvent* ev)
{
...
void QPropertyTreeLegacy::mouseDoubleClickEvent(QMouseEvent* ev)
{
...
void QPropertyTreeLegacy::onMouseStillTimer()
{
	onMouseStill();
}

void QPropertyTreeLegacy::flushAggregatedMouseEvents()
{
...
void QPropertyTreeLegacy::mouseMoveEvent(QMouseEvent* ev)
{
...
void QPropertyTreeLegacy::wheelEvent(QWheelEvent* ev) 
{
...
bool QPropertyTreeLegacy::_isDragged(const PropertyRow* row) const
{
...
void QPropertyTreeLegacy::onAttachedTreeChanged()
{
	revert();
}

void QPropertyTreeLegacy::setSizeToContent(bool sizeToContent)
{
...
void QPropertyTreeLegacy::_cancelWidget()
{
	QWidget* inplaceWidget = 0;
	if (widget_.get() && widget_->actualWidget())
		inplaceWidget = (QWidget*)widget_->actualWidget();
	...
	PropertyTreeLegacy::_cancelWidget();
}

bool QPropertyTreeLegacy::focusNextPrevChild(bool next)
{
	PropertyRow* row = model()->focusedRow();
	if (!row)
...
void QPropertyTreeLegacy::HandleDragDrop(QDropEvent* ev, PropertyRowDragDropHandler handler)
{
...
void QPropertyTreeLegacy::dragEnterEvent(QDragEnterEvent* ev)
{
	HandleDragDrop(ev, &PropertyRow::onDataDragMove);
}

void QPropertyTreeLegacy::dragMoveEvent(QDragMoveEvent* ev)
{
	HandleDragDrop(ev, &PropertyRow::onDataDragMove);
}

void QPropertyTreeLegacy::dropEvent(QDropEvent* ev)
{
