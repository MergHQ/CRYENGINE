...
public:
	...
	void                    setExpandedRecursive(PropertyTreeLegacy* tree, bool expanded);
	...
	bool                    visible(const PropertyTreeLegacy* tree) const;
	bool                    hasVisibleChildren(const PropertyTreeLegacy* tree, bool internalCall = false) const;

	const PropertyRow*      hit(const PropertyTreeLegacy* tree, Point point) const;
	PropertyRow*            hit(const PropertyTreeLegacy* tree, Point point);
	...
	template<class Op> bool scanChildren(Op& op, PropertyTreeLegacy* tree);
	template<class Op> bool scanChildrenReverse(Op& op, PropertyTreeLegacy* tree);
	template<class Op> bool scanChildrenBottomUp(Op& op, PropertyTreeLegacy* tree);
	...
	void                    updateLabel(const PropertyTreeLegacy* tree, int index, bool parentHidesNonInlineChildren);
	void                    updateTextSizeInitial(const PropertyTreeLegacy* tree, int index, bool force);
	virtual void            labelChanged()                    {}
	void                    parseControlCodes(const PropertyTreeLegacy* tree, const char* label, bool changeLabel);
	const char*             typeName() const                  { return typeName_; }
	virtual const char*     typeNameForFilter(PropertyTreeLegacy* tree) const;
	void                    setTypeName(const char* typeName) { YASLI_ASSERT(strlen(typeName)); typeName_ = typeName; }
	const char* rowText(char (&containerLabelBuffer)[16], const PropertyTreeLegacy* tree, int rowIndex) const;
	...
	int                                   verticalIndex(PropertyTreeLegacy* tree, PropertyRow* row);
	PropertyRow*                          rowByVerticalIndex(PropertyTreeLegacy* tree, int index);
	int                                   horizontalIndex(PropertyTreeLegacy* tree, PropertyRow* row);
	PropertyRow*                          rowByHorizontalIndex(PropertyTreeLegacy* tree, int index);
	...
	virtual int                           widgetSizeMin(const PropertyTreeLegacy*) const { return userWidgetSize() >= 0 ? userWidgetSize() : 0; }
	virtual int                           widgetSizeMax(const PropertyTreeLegacy*) const { return INT_MAX; }
	virtual int                           floorHeight() const                      { return 0; }

	void                                  calcPulledRows(int* minTextSize, int* freePulledChildren, int* minimalWidth, const PropertyTreeLegacy* tree, int index);
	void                                  calculateMinimalSize(const PropertyTreeLegacy* tree, int posX, int availableWidth, bool force, int* extraSizeRemainder, int* _extraSize, int index);
	void                                  setTextSize(const PropertyTreeLegacy* tree, int rowIndex, float multiplier);
	void                                  adjustVerticalPosition(const PropertyTreeLegacy* tree, int& totalHeight);
	...
	Rect                                  rectIncludingChildren(const PropertyTreeLegacy* tree) const;
	Rect                                  textRect(const PropertyTreeLegacy* tree) const;
	Rect                                  widgetRect(const PropertyTreeLegacy* tree) const;
	Rect                                  plusRect(const PropertyTreeLegacy* tree) const;
	Rect                                  floorRect(const PropertyTreeLegacy* tree) const;
	Rect                                  validatorRect(const PropertyTreeLegacy* tree) const;
	Rect                                  validatorWarningIconRect(const PropertyTreeLegacy* tree) const;
	Rect                                  validatorErrorIconRect(const PropertyTreeLegacy* tree) const;
	Rect                                  titleIconRect(const PropertyTreeLegacy* tree) const;
	int                                   heightIncludingChildren() const { return heightIncludingChildren_; }
	property_tree::Font                   rowFont(const PropertyTreeLegacy* tree) const;

	void                                  drawRow(IDrawContext& x, const PropertyTreeLegacy* tree, int rowIndex, bool selectionPass);

	virtual void                          redraw(IDrawContext& context);
	virtual property_tree::InplaceWidget* createWidget(PropertyTreeLegacy* tree)                               { return 0; }
	...
	bool                                  canBeToggled(const PropertyTreeLegacy* tree) const;
	bool                                  canBeDragged() const;
	bool                                  canBeDroppedOn(const PropertyRow* parentRow, const PropertyRow* beforeChild, const PropertyTreeLegacy* tree) const;
	void                                  dropInto(PropertyRow* parentRow, PropertyRow* cursorRow, PropertyTreeLegacy* tree, bool before);
	virtual bool                          getHoverInfo(PropertyHoverInfo* hit, const Point& cursorPos, const PropertyTreeLegacy* tree) const
	{
	...
	virtual bool           onKeyDown(PropertyTreeLegacy* tree, const KeyEvent* ev);
	virtual bool           onMouseDown(PropertyTreeLegacy* tree, Point point, bool& changed) { return false; }
	virtual void           onMouseDrag(const PropertyDragEvent& e)                     {}
	virtual void           onMouseStill(const PropertyDragEvent& e)                    {}
	virtual void           onMouseUp(PropertyTreeLegacy* tree, Point point)                  {}
	...
	virtual bool           onMouseDragCheck(PropertyTreeLegacy* tree, bool value)            { return false; }
	virtual bool           onContextMenu(IMenu& menu, PropertyTreeLegacy* tree);

	virtual bool           onDataDragMove(PropertyTreeLegacy* tree);
	virtual bool           onDataDrop(PropertyTreeLegacy* tree);

	virtual bool           isFullRow(const PropertyTreeLegacy* tree) const;
	...
	bool                          isGroupParent(const PropertyTreeLegacy* tree) const;
...
struct RowWidthCache
{
	...
	int getOrUpdate(const PropertyTreeLegacy* tree, const PropertyRow* rowForValue, int extraSpace);
};
...
template<class Op>
bool PropertyRow::scanChildren(Op& op, PropertyTreeLegacy* tree)
{
...
template<class Op>
bool PropertyRow::scanChildrenReverse(Op& op, PropertyTreeLegacy* tree)
{
	int numChildren = (int)children_.size();
...
template<class Op>
bool PropertyRow::scanChildrenBottomUp(Op& op, PropertyTreeLegacy* tree)
{
