...
struct ClassMenuItemAdderRowPointer : ClassMenuItemAdder{
	ClassMenuItemAdderRowPointer(PropertyRowPointer* row, PropertyTreeLegacy* tree) : row_(row), tree_(tree) {}    
...
bool PropertyRowPointer::onMouseDown(PropertyTreeLegacy* tree, Point point, bool& changed) 
{
...
bool PropertyRowPointer::onContextMenu(IMenu &menu, PropertyTreeLegacy* tree)
{
	if(!menu.isEmpty())
		menu.addSeparator();
		if(!userReadOnly()){
...
int PropertyRowPointer::widgetSizeMin(const PropertyTreeLegacy* tree) const
{
	if (userWidgetSize() >= 0)
	{
