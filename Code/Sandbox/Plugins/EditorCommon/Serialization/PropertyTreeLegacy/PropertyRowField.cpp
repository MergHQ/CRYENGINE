...
Rect PropertyRowField::fieldRect(const PropertyTreeLegacy* tree) const
{
...
bool PropertyRowField::onMouseDown(PropertyTreeLegacy* tree, Point point, bool& changed)
{
	changed = false;
...
void PropertyRowField::onMouseUp(PropertyTreeLegacy* tree, Point point)
{
	tree->ui()->unsetCursor();
}
...
Icon PropertyRowField::buttonIcon(const PropertyTreeLegacy* tree, int index) const
{
	return Icon();
}

int PropertyRowField::widgetSizeMin(const PropertyTreeLegacy* tree) const
{ 
	if (userWidgetSize() >= 0)
...
int PropertyRowField::hitButton(const PropertyTreeLegacy* tree, const Point& point) const
{
	int buttonCount = this->buttonCount();
