...
void PropertyRow::setExpandedRecursive(PropertyTreeLegacy* tree, bool expanded)
{
...
void PropertyRow::updateLabel(const PropertyTreeLegacy* tree, int index, bool parentHidesNonInlineChildren)
{
	if (!labelChanged_) {
		if (pulledUp_)
...
void PropertyRow::parseControlCodes(const PropertyTreeLegacy* tree, const char* ptr, bool changeLabel)
{
	if (changeLabel) {
		userFullRow_ = false;
		pulledUp_ = false;
...
const char* PropertyRow::typeNameForFilter(PropertyTreeLegacy* tree) const
{
	return typeName();
}

void PropertyRow::updateTextSizeInitial(const PropertyTreeLegacy* tree, int index, bool fontChanged)
{
...
void PropertyRow::calculateMinimalSize(const PropertyTreeLegacy* tree, int posX, int availableWidth, bool force, int* _extraSizeRemainder, int* _extraSize, int index)
{
...
void PropertyRow::adjustVerticalPosition(const PropertyTreeLegacy* tree, int& totalHeight)
{
...
void PropertyRow::setTextSize(const PropertyTreeLegacy* tree, int index, float mult)
{
...
void PropertyRow::calcPulledRows(int* minTextSize, int* freePulledChildren, int* minimalWidth, const PropertyTreeLegacy *tree, int index) 
{
	updateTextSizeInitial(tree, index, false);
...
bool PropertyRow::onKeyDown(PropertyTreeLegacy* tree, const KeyEvent* ev)
{
...
struct ExpansionMenuHandler : PropertyRowMenuHandler
{
	...
	ExpansionMenuHandler(PropertyTreeLegacy* tree, PropertyRow* row)
	: tree(tree), row(row)
	{
	}
...
bool PropertyRow::onContextMenu(property_tree::IMenu &menu, PropertyTreeLegacy* tree)
{
	if(parent() && parent()->isContainer()){
		PropertyRowContainer* container = static_cast<PropertyRowContainer*>(parent());
...
bool PropertyRow::onDataDragMove(PropertyTreeLegacy* tree)
{
	// By default, do not accept any drag&drop operations.
	return false;
}

bool PropertyRow::onDataDrop(PropertyTreeLegacy* tree)
{
	// By default, do not accept any drag&drop operations.
	return false;
}
...
Font PropertyRow::rowFont(const PropertyTreeLegacy* tree) const
{
...
Rect PropertyRow::rectIncludingChildren(const PropertyTreeLegacy* tree) const
{
	Rect r = rect();
	if (expanded())
...
bool PropertyRow::isGroupParent(const PropertyTreeLegacy* tree) const
{
	if (!parent() || !parent()->isRoot())
		return false;
...
void PropertyRow::drawRow(IDrawContext& context, const PropertyTreeLegacy* tree, int index, bool selectionPass)
{
	Rect rowRect = rect();
	Rect selectionRect;
...
bool PropertyRow::visible(const PropertyTreeLegacy* tree) const
{
...
bool PropertyRow::canBeToggled(const PropertyTreeLegacy* tree) const
{
	if(!visible(tree))
		return false;
...
bool PropertyRow::canBeDroppedOn(const PropertyRow* parentRow, const PropertyRow* beforeChild, const PropertyTreeLegacy* tree) const
{
...
void PropertyRow::dropInto(PropertyRow* parentRow, PropertyRow* cursorRow, PropertyTreeLegacy* tree, bool before)
{
...
const char* PropertyRow::rowText(char (&containerLabelBuffer)[16], const PropertyTreeLegacy* tree, int index) const
{
...
bool PropertyRow::hasVisibleChildren(const PropertyTreeLegacy* tree, bool internalCall) const
{
...
const PropertyRow* PropertyRow::hit(const PropertyTreeLegacy* tree, Point point) const
{
  return const_cast<PropertyRow*>(this)->hit(tree, point);
}

PropertyRow* PropertyRow::hit(const PropertyTreeLegacy* tree, Point point)
{
...
int PropertyRow::verticalIndex(PropertyTreeLegacy* tree, PropertyRow* row)
{
...
PropertyRow* PropertyRow::rowByVerticalIndex(PropertyTreeLegacy* tree, int index)
{
...
int PropertyRow::horizontalIndex(PropertyTreeLegacy* tree, PropertyRow* row)
{
	if(row == this)
		return 0;
...
PropertyRow* PropertyRow::rowByHorizontalIndex(PropertyTreeLegacy* tree, int index)
{
	if(!index)
		return this;
...
bool PropertyRow::isFullRow(const PropertyTreeLegacy* tree) const
{
    if (tree->fullRowMode())
		return true;
...
Rect PropertyRow::textRect(const PropertyTreeLegacy* tree) const
{
	return Rect(textPos_, pos_.y(), textSize_ < textSizeInitial_ ? textSize_ - 1 : textSize_, tree->_defaultRowHeight());
}

Rect PropertyRow::widgetRect(const PropertyTreeLegacy* tree) const
{
	return Rect(widgetPos_, pos_.y(), widgetSize_, tree->_defaultRowHeight());
}

Rect PropertyRow::validatorRect(const PropertyTreeLegacy* tree) const
{
	return Rect(pos_.x() + plusSize_, pos_.y() + size_.y() - validatorsHeight_, size_.x() - plusSize_, validatorsHeight_);
}

Rect PropertyRow::validatorErrorIconRect(const PropertyTreeLegacy* tree) const
{
...
Rect PropertyRow::validatorWarningIconRect(const PropertyTreeLegacy* tree) const
{
...
Rect PropertyRow::plusRect(const PropertyTreeLegacy* tree) const
{
	return Rect(pos_.x(), pos_.y(), plusSize_, tree->_defaultRowHeight());
}

Rect PropertyRow::floorRect(const PropertyTreeLegacy* tree) const
{
...
Rect PropertyRow::titleIconRect(const PropertyTreeLegacy* tree) const
{
	int iconSize = tree->_defaultRowHeight();
...
int RowWidthCache::getOrUpdate(const PropertyTreeLegacy* tree, const PropertyRow* rowForValue, int extraSpace)
{
