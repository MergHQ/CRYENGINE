...
public:
	...
	void populateComboBox(std::vector<std::string>* values, int* selectedIndex, PropertyTreeLegacy* tree) override
	{
		values->assign(stringList_.begin(), stringList_.end());
		*selectedIndex = stringList_.find(value_.c_str());
	}

	bool onComboBoxSelected(const char* newValue, PropertyTreeLegacy* tree) override
	{
		if (value_ != newValue) {
...
public:
	InplaceWidget* createWidget(PropertyTreeLegacy* tree) override;
