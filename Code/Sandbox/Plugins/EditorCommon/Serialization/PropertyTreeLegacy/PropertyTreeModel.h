...
public:
	...
	PropertyTreeModel(PropertyTreeLegacy* tree);
	~PropertyTreeModel();
	...
	void undo();
	void clearUndo();
	//! Notify the property tree that the latest undo action has ended
	//! \param undoAccepted if the undo action is accepted
	void               endUndo(bool undoAccepted);
	...
	void               YASLI_SERIALIZE_METHOD(yasli::Archive& ar, PropertyTreeLegacy* tree);
	...
	// Be careful: these functions can destroy 'row'
	//! Called when a row has changed, updates the row and ends the current undo action
	void rowChanged(PropertyRow* row, bool apply = true, bool shouldAcceptUndo = true);
	//! Called when a row is changing continuously, updates the row, but does not end current undo action
	void rowContinuouslyChanged(PropertyRow* row, bool apply = true);
	//! Applies change to row data (like label, layout and multivalue) and sends row updated signal (currently intercepted by property tree, which triggers update and revert)
	void updateRow(PropertyRow* row, bool apply = true);
...
private:
	...
	void         beginUndo(const PropertyTreeOperator& op);
