...
PropertyTreeStyle PropertyTreeLegacy::defaultTreeStyle_;

PropertyTreeLegacy::PropertyTreeLegacy(IUIFacade* uiFacade)
...
PropertyTreeLegacy::~PropertyTreeLegacy()
{
	clearMenuHandlers();
	delete ui_;
}

bool PropertyTreeLegacy::onRowKeyDown(PropertyRow* row, const KeyEvent* ev)
{
...
struct FirstIssueVisitor
{
	...
	ScanResult operator()(PropertyRow* row, PropertyTreeLegacy* tree, int)
	{
...
void PropertyTreeLegacy::jumpToNextHiddenValidatorIssue(bool isError, PropertyRow* start)
{
...
bool PropertyTreeLegacy::onRowLMBDown(PropertyRow* row, const Rect& rowRect, Point point, bool controlPressed, bool shiftPressed)
{
	pressPoint_ = point;
...
void PropertyTreeLegacy::onMouseStill()
{
	if (capturedRow_)
	{
		PropertyDragEvent e;
...
void PropertyTreeLegacy::onRowLMBUp(PropertyRow* row, const Rect& rowRect, Point point)
{
...
void PropertyTreeLegacy::ShowContexMenu(PropertyRow* row, const Rect& rowRect, Point point)
{
...
void PropertyTreeLegacy::expandParents(PropertyRow* row)
{
...
void PropertyTreeLegacy::expandAll()
{
	expandChildren(0);
}

void PropertyTreeLegacy::expandChildren(PropertyRow* root)
{
	if (!root)
	{
		root = model()->root();
...
void PropertyTreeLegacy::collapseAll()
{
	collapseChildren(0);
}

void PropertyTreeLegacy::collapseChildren(PropertyRow* root)
{
	if (!root)
	{
		root = model()->root();
...
void PropertyTreeLegacy::expandRow(PropertyRow* row, bool expanded, bool updateHeights)
{
	bool hasChanges = false;
...
Point PropertyTreeLegacy::treeSize() const
{
	return size_ + (compact() ? Point(0, 0) : Point(8, 8));
}

void PropertyTreeLegacy::YASLI_SERIALIZE_METHOD(Archive& ar)
{
...
void PropertyTreeLegacy::ensureVisible(PropertyRow* row, bool update, bool considerChildren)
{
	if (row == 0)
		return;
	if (row->isRoot())
...
void PropertyTreeLegacy::onRowSelected(const std::vector<PropertyRow*>& rows, bool addSelection, bool adjustCursorPos)
{
...
bool PropertyTreeLegacy::attach(const yasli::Serializers& serializers)
{
	bool changed = false;
...
void PropertyTreeLegacy::attach(const yasli::Serializer& serializer)
{
...
void PropertyTreeLegacy::attach(const yasli::Object& object)
{
	attached_.clear();
...
void PropertyTreeLegacy::detach()
{
...
void PropertyTreeLegacy::_cancelWidget()
{
...
int PropertyTreeLegacy::revertObjects(vector<void*> objectAddresses)
{
	int result = 0;
...
bool PropertyTreeLegacy::revertObject(void* objectAddress)
{
...
void PropertyTreeLegacy::revert()
{
	interruptDrag();
	_cancelWidget();
	capturedRow_ = 0;
...
void PropertyTreeLegacy::applyValidation()
{
...
void PropertyTreeLegacy::revertNoninterrupting()
{
	if (!capturedRow_)
		revert();
}

void PropertyTreeLegacy::apply(bool continuous)
{
	//QElapsedTimer timer;
	//timer.start();
...
void PropertyTreeLegacy::applyInplaceEditor()
{
	if (widget_.get())
		widget_->commit();
}

bool PropertyTreeLegacy::spawnWidget(PropertyRow* row, bool ignoreReadOnly)
{
...
void PropertyTreeLegacy::addMenuHandler(PropertyRowMenuHandler* handler)
{
	menuHandlers_.push_back(handler);
}

void PropertyTreeLegacy::clearMenuHandlers()
{
...
bool PropertyTreeLegacy::onContextMenu(PropertyRow* r, IMenu& menu)
{
...
void PropertyTreeLegacy::onRowMouseMove(PropertyRow* row, const Rect& rowRect, Point point, Modifier modifiers)
{
	PropertyDragEvent e;
	e.tree = this;
...
void PropertyTreeLegacy::onRowMenuDecompose(PropertyRow* row)
{
...
void PropertyTreeLegacy::onModelUpdated(const PropertyRows& rows, bool needApply)
{
	_cancelWidget();
...
void PropertyTreeLegacy::onModelBeginUndo(PropertyTreeOperator* op, bool* handled)
{
	onBeginUndo();
}

void PropertyTreeLegacy::onModelEndUndo(bool undoAccepted)
{
	onEndUndo(undoAccepted);
}

void PropertyTreeLegacy::setWidget(property_tree::InplaceWidget* widget, PropertyRow* widgetRow)
{
	_cancelWidget();
...
void PropertyTreeLegacy::setExpandLevels(int levels)
{
	config_.expandLevels = levels;
	model()->setExpandLevels(levels);
}

PropertyRow* PropertyTreeLegacy::selectedRow()
{
...
bool PropertyTreeLegacy::getSelectedObject(yasli::Object* object)
{
	const PropertyTreeModel::Selection& sel = model()->selection();
	if (sel.empty())
		return 0;
...
bool PropertyTreeLegacy::setSelectedRow(PropertyRow* row)
{
	TreeSelection sel;
	if (row)
...
int PropertyTreeLegacy::selectedRowCount() const
{
	return (int)model()->selection().size();
}

PropertyRow* PropertyTreeLegacy::selectedRowByIndex(int index)
{
	const PropertyTreeModel::Selection& sel = model()->selection();
...
bool PropertyTreeLegacy::selectByAddress(const void* addr, bool keepSelectionIfChildSelected)
{
	return selectByAddresses(vector<const void*>(1, addr), keepSelectionIfChildSelected);
}

bool PropertyTreeLegacy::selectByAddresses(const vector<const void*>& addresses, bool keepSelectionIfChildSelected)
{
	return selectByAddresses(&addresses.front(), addresses.size(), keepSelectionIfChildSelected);
}

bool PropertyTreeLegacy::selectByAddresses(const void* const* addresses, size_t addressCount, bool keepSelectionIfChildSelected)
{
	bool result = false;
	if (model()->root())
	{
...
void PropertyTreeLegacy::setUndoEnabled(bool enabled, bool full)
{
...
void PropertyTreeLegacy::attachPropertyTree(PropertyTreeLegacy* propertyTree)
{
	// TODO:
	// if(attachedPropertyTree_)
...
void PropertyTreeLegacy::detachPropertyTree()
{
	attachPropertyTree(0);
}

void PropertyTreeLegacy::setAutoHideAttachedPropertyTree(bool autoHide)
{
	autoHideAttachedPropertyTree_ = autoHide;
}

void PropertyTreeLegacy::getSelectionSerializers(yasli::Serializers* serializers)
{
...
void PropertyTreeLegacy::updateAttachedPropertyTree(bool revert)
{
	if (attachedPropertyTree_)
	{
...
void PropertyTreeLegacy::RowFilter::parse(const char* filter)
{
...
bool PropertyTreeLegacy::RowFilter::match(const char* textOriginal, Type type, size_t* matchStart, size_t* matchEnd) const
{
...
PropertyRow* PropertyTreeLegacy::rowByPoint(const Point& pt)
{
	if (!model_->root())
		return 0;
...
Point PropertyTreeLegacy::pointToRootSpace(const Point& point) const
{
	return Point(point.x() + offset_.x(), point.y() + offset_.y());
}

Point PropertyTreeLegacy::pointFromRootSpace(const Point& point) const
{
	return Point(point.x() - offset_.x() + area_.left(), point.y() - offset_.y() + area_.top());
}

bool PropertyTreeLegacy::toggleRow(PropertyRow* row)
{
...
bool PropertyTreeLegacy::_isCapturedRow(const PropertyRow* row) const
{
	return capturedRow_ == row;
}

void PropertyTreeLegacy::setValueColumnWidth(float valueColumnWidth)
{
...
void PropertyTreeLegacy::setArchiveContext(yasli::Context* lastContext)
{
	archiveContext_ = lastContext;
}

void PropertyTreeLegacy::onAttachedTreeChanged()
{
	revert();
}

Point PropertyTreeLegacy::_toWidget(Point point) const
{
	return Point(point.x() - offset_.x(), point.y() - offset_.y());
}
...
void PropertyTreeLegacy::updateValidatorIcons()
{
	if (!validatorBlock_->isEnabled())
		return;
...
void PropertyTreeLegacy::setDefaultTreeStyle(const PropertyTreeStyle& treeStyle)
{
	defaultTreeStyle_ = treeStyle;
}

void PropertyTreeLegacy::setTreeStyle(const PropertyTreeStyle& style)
{
	*style_ = style;
	updateHeights(true);
}

void PropertyTreeLegacy::setPackCheckboxes(bool pack)
{
	style_->packCheckboxes = pack;
	updateHeights(true);
}

bool PropertyTreeLegacy::packCheckboxes() const
{
	return style_->packCheckboxes;
}

void PropertyTreeLegacy::setCompact(bool compact)
{
	style_->compact = compact;
	repaint();
}

bool PropertyTreeLegacy::compact() const
{
	return style_->compact;
}

void PropertyTreeLegacy::setRowSpacing(float rowSpacing)
{
	style_->rowSpacing = rowSpacing;
}

float PropertyTreeLegacy::rowSpacing() const
{
	return style_->rowSpacing;
}

float PropertyTreeLegacy::valueColumnWidth() const
{
	return style_->valueColumnWidth;
}

void PropertyTreeLegacy::setFullRowMode(bool fullRowMode)
{
	style_->fullRowMode = fullRowMode;
	repaint();
}

bool PropertyTreeLegacy::fullRowMode() const
{
	return style_->fullRowMode;
}

bool PropertyTreeLegacy::containsWarnings() const
{
	return validatorBlock_->containsWarnings();
}

bool PropertyTreeLegacy::containsErrors() const
{
	return validatorBlock_->containsErrors();
}

void PropertyTreeLegacy::focusFirstError()
{
	jumpToNextHiddenValidatorIssue(true, model()->root());
}

Rect PropertyTreeLegacy::getPropertySplitterRect() const
{
...
void PropertyTreeLegacy::setPropertySplitterPos(int pos)
{
