	...
	, m_bDisableTimerHide(false)
{
	...
	if (pNotificationDesc->GetCommandCount())
		messageText += "<a style=\"color:" + GetStyleHelper()->selectedIconTint().name() + ";\" href=\"#execute_commands\">Go to error</a>";
	...
	if (pNotificationDesc->GetType() == Internal::CNotification::Progress)
	{
	...
	else
	{
		...
		switch (pNotificationDesc->GetType())
		{
