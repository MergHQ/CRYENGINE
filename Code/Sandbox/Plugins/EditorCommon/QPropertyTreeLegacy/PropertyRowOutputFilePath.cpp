...
OutputFilePathMenuHandler::OutputFilePathMenuHandler(PropertyTreeLegacy* tree, PropertyRowOutputFilePath* self)
	: self(self), tree(tree)
{
}
...
bool PropertyRowOutputFilePath::onActivate(const PropertyActivationEvent& e)
{
	...
	QString result = CEngineFileDialog::RunGameSave(runParams, static_cast<QPropertyTreeLegacy*>(e.tree));
	if (!result.isEmpty())
	{
...
Icon PropertyRowOutputFilePath::buttonIcon(const PropertyTreeLegacy* tree, int index) const
{
#include "Serialization/PropertyTreeLegacy/file_save.xpm"
...
bool PropertyRowOutputFilePath::onContextMenu(IMenu& menu, PropertyTreeLegacy* tree)
{
