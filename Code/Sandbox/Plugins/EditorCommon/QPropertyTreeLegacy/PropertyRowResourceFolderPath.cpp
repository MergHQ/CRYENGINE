...
ResourceFolderPathMenuHandler::ResourceFolderPathMenuHandler(PropertyTreeLegacy* tree, PropertyRowResourceFolderPath* self)
	: self(self), tree(tree)
{
}
...
bool PropertyRowResourceFolderPath::onActivate(const PropertyActivationEvent& e)
{
	...
	QString result = CEngineFileDialog::RunGameSelectDirectory(runParams, static_cast<QPropertyTreeLegacy*>(e.tree));
	if (result.isEmpty())
	{
		return true;
	}
...
property_tree::Icon PropertyRowResourceFolderPath::buttonIcon(const PropertyTreeLegacy* tree, int index) const
{
#include "Serialization/PropertyTreeLegacy/file_open.xpm"
...
bool PropertyRowResourceFolderPath::onContextMenu(IMenu& menu, PropertyTreeLegacy* tree)
{
