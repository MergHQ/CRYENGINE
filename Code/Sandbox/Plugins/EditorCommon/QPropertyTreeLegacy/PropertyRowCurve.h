...
public:
	...
	int             widgetSizeMin(const PropertyTreeLegacy* tree) const override { return userWidgetSize() >= 0 ? userWidgetSize() : 48; }
	...
	// additional
	void          editCurve(PropertyTreeLegacy* tree);
	PropertyTreeLegacy* tree() const { return tree_; }
...
struct CurveMenuHandler : PropertyRowMenuHandler
{
	...
	CurveMenuHandler(PropertyTreeLegacy* tree, PropertyRowCurve* propertyRowCurve)
