...
CItemModelAttribute* CAssetModel::GetColumnAttribute(int column)
{
	switch (column)
	{
...
QVariant CAssetModel::data(const QModelIndex& index, int role) const
{
	...
	{
		return QString(CryGUIDHelper::Print(asset->GetGUID()));
	}
	default:
		break;
	}
	...
	if (role == Qt::DisplayRole)
	{
		...
		if (detailColumn >= 0 && detailColumn < m_detailAttributes.size())
		{
			...
			QVariant value(pAssetType->GetDetailValue(asset, m_detailAttributes[detailColumn].pAttribute));
...
{
	return m_favHelper.flags(index) | Qt::ItemIsDragEnabled;
}
...
QMimeData* CAssetModel::mimeData(const QModelIndexList& indexes) const
{
	...
	for (const auto& index : indexes)
	{
		...
		auto filePath = QtUtil::ToQString(pAsset->GetFile(0));
