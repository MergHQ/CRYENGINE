...
namespace LODGenerator 
{
	...
	bool CAutoGeneratorTextureCreator::GenerateTexture(int level)
	{
		...
		for ( int idx = 0; idx < submatCount && idx < MAX_MAT_SLOTS; ++idx)
		{
			...
			{
				if (QDialogButtonBox::StandardButton::No == CQuestionDialog::SQuestion("UV Layout", "Detected overlapping UVs in target mesh. This could cause artifacts. Continue anyway?"))
					return false;
			}
		}
	...
	bool CAutoGeneratorTextureCreator::SaveTextures(const int level)
	{
		...
		string exportPath = autoGeneratorParams.GetMaterialOption<string>("ExportPath");
		bool bBakeAlpha = autoGeneratorParams.GetMaterialOption<bool>("BakeAlpha");
		string filenamePattern = autoGeneratorParams.GetMaterialOption<string>("FilenameTemplate");
		...
		for (int i=CAutoGeneratorDataBase::eTextureType_Diffuse; i<CAutoGeneratorDataBase::eTextureType_Max; i++)
		{
			...
			string cPath = CAutoGeneratorHelper::GetDefaultTextureName(i,level,bBakeAlpha,exportPath,filenamePattern);
			cPath = PathUtil::Make(PathUtil::GetGameProjectAssetsPath(), cPath);
			CFileUtil::CreatePath(PathUtil::GetParentDirectory(cPath));
			bool checkedOut = CAutoGeneratorHelper::CheckoutOrExtract(cPath);
			string preset = GetPreset(i);
	...
	string CAutoGeneratorTextureCreator::GetPreset(const int texturetype)
	{
		CAutoGeneratorParams& autoGeneratorParams = CAutoGeneratorDataBase::Instance()->GetParams();
		...
		switch(texturetype)
		{
		case CAutoGeneratorDataBase::eTextureType_Diffuse: preset = autoGeneratorParams.GetMaterialOption<string>("DiffuseTexturePreset");
			break;
		case CAutoGeneratorDataBase::eTextureType_Normal: preset = autoGeneratorParams.GetMaterialOption<string>("NormalTexturePreset");
			break;
		case CAutoGeneratorDataBase::eTextureType_Spec: preset = autoGeneratorParams.GetMaterialOption<string>("SpecularTexturePreset");
			break;
		}
	...
	void CAutoGeneratorTextureCreator::AssignToMaterial(const int nLod, const bool bAlpha, const bool bSaveSpec, const string& exportPath, const string& fileNameTemplate)
	{
		CAutoGeneratorDataBase* pAutoGeneratorDataBase = CAutoGeneratorDataBase::Instance();
		CAutoGeneratorParams& autoGeneratorParams = CAutoGeneratorDataBase::Instance()->GetParams();
		...
		if(pMtl)
		{
			...
			if(!pMtl->Save(false))
			{
				CQuestionDialog::SWarning("Material File Save Failed", "The material file cannot be saved. The file is located in a PAK archive or access is denied");
			}
	...
	bool CAutoGeneratorTextureCreator::SaveTexture(ITexture* pTex,const bool bAlpha,const string& fileName,const string& texturePreset)
	{
		if (!pTex)
			return false;
