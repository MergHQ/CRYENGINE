...
namespace LODGenerator
{
	...
	{
		// To avoid having to include massive amounts of third party code (around 1000 files),
		// the support for ABF method is dropped.
	...
	IStatObj* AutoUV::MakeMesh(std::vector<SAutoUVFinalVertex> &vertices, std::vector<int> &indices)
	{
	...
	void AutoUV::Render()
	{
		...
		for (std::vector<SAutoUVSquare>::iterator it=m_squares.begin(), end=m_squares.end(); it!=end; ++it)
		{
			...
			{
				SAutoUVFinalVertex &a=m_outVertices[m_outIndices[3*i+0]];
				SAutoUVFinalVertex &b=m_outVertices[m_outIndices[3*i+1]];
				SAutoUVFinalVertex &c=m_outVertices[m_outIndices[3*i+2]];
