...
namespace LODGenerator 
{
	...
	public:
		...
		void SetGeometryOptionLimit(string paramName, float fMin, float fMax, float fStep = 0.f, bool bHardMin = true, bool bHardMax = true )
		{
			if (IVariable* pVar = m_pGeometryVarBlock->FindVariable(paramName))
				pVar->SetLimits(fMin,fMax,fStep,bHardMin,bHardMax);
		}

		void SetMaterialOptionLimit(string paramName, float fMin, float fMax, float fStep = 0.f, bool bHardMin = true, bool bHardMax = true )
		{
			if (IVariable* pVar = m_pMaterialVarBlock->FindVariable(paramName))
				pVar->SetLimits(fMin,fMax,fStep,bHardMin,bHardMax);
		}

		void GetGeometryOptionLimit(string paramName, float& fMin, float& fMax, float& fStep, bool& bHardMin, bool& bHardMax )
		{
			if (IVariable* pVar = m_pGeometryVarBlock->FindVariable(paramName))
				pVar->GetLimits(fMin,fMax,fStep,bHardMin,bHardMax);
		}

		void GetMaterialOptionLimit(string paramName, float& fMin, float& fMax, float& fStep, bool& bHardMin, bool& bHardMax )
		{
			if (IVariable* pVar = m_pMaterialVarBlock->FindVariable(paramName))
				pVar->GetLimits(fMin,fMax,fStep,bHardMin,bHardMax);
		}
