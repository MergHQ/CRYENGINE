...
{
	string CAutoGeneratorHelper::m_changeId = "";
	...
	bool CAutoGeneratorHelper::CheckoutInSourceControl(const string& filename)
	{
	...
	string CAutoGeneratorHelper::GetDefaultTextureName(const int i, const int nLod, const bool bAlpha, const string& exportPath, const string& fileNameTemplate)
	{
		...
		outName=PathUtil::MakeGamePath(outName);
		return outName;
	}
