...
namespace CryEngine
{
	...
	[EntityClass("Animator", "Game", null, IconType.User)]
	/// <summary>
	/// Helper class for entities to easily manager animations.
	/// </summary>
	public class CharacterAnimator : EntityComponent
	{
		...
		private SAnimationContext _animationContext;
		...
		{ 
			get
			{
				...
				return _character.NativeHandle.GetPlaybackScale();
			}
			set
			{
				if(_character != null)
				{
					_character.NativeHandle.SetPlaybackScale(value);
				}
		...
		private void Deinitialize()
		{
			if(_character != null)
			{
				var skeleton = _character?.NativeHandle.GetISkeletonAnim();
				if(skeleton != null)
				{
				...
				_character.NativeHandle?.Dispose();
				_character = null;
			}
		...
		private void Initialize(bool silent)
		{
			...
			var mannequinInterface = Engine.GameFramework.GetMannequinInterface();
			var animationDatabaseManager = mannequinInterface.GetAnimationDatabaseManager();


			// Load the Mannequin controller definition.
			// This is owned by the animation database manager
			var controllerDefinition = animationDatabaseManager.LoadControllerDef(mannequinControllerDefinition);
			...
			_animationContext = new SAnimationContext(controllerDefinition);

			// Now create the controller that will be handling animation playback
			_actionController = mannequinInterface.CreateActionController(entity.NativeHandle, _animationContext);
			...
			ActivateMannequinContext(mannequinContextName, character.NativeHandle, controllerDefinition, animationDatabase);
			...
			int priority = 0;
			var idleFragmentId = controllerDefinition.m_fragmentIDs.Find(startFragmentName);
			...
			var tagState = new TagState(ETagStateEmpty.TAG_STATE_EMPTY);
			var animationContextList = AnimationContextActionList.CreateSAnimationContext(priority, idleFragmentId, tagState, 0, 0, 0);

			// Queue the idle fragment to start playing immediately on next update
			_actionController.Queue(animationContextList);
			...
			character.NativeHandle.SetPlaybackScale(_playbackScale);
		}

		private void ActivateMannequinContext(string contextName, ICharacterInstance character, SControllerDef controllerDefinition, IAnimationDatabase animationDatabase)
		{
			var entity = Entity;

			var scopeContextId = controllerDefinition.m_scopeContexts.Find(contextName);
			...
			_actionController.SetScopeContext((uint)scopeContextId, entity.NativeHandle, character, animationDatabase);
		}
		...
		private void ApplyTagValues()
		{
			...
			var state = _animationContext?.state;
			if(state == null)
			{
				Log.Error<CharacterAnimator>("Unable to apply tag values because the State could not be retrieved from the AnimationContext!");
				return;
			}

			foreach(var keyValue in _tagValues)
			{
				state.Set(keyValue.Key, keyValue.Value);
			}
			_tagValues.Clear();
		}
		...
		private void ApplyMotionValues()
		{
			...
			{
				skeleton.SetDesiredMotionParam((EMotionParamID)keyValue.Key, keyValue.Value, 0);
			}
			_motionValues.Clear();
		}
		...
		public AnimationTag FindTag(string name)
		{
			...
			var id = _animationContext.state.GetDef().Find(name);
			...
			if(id < 0)
			{
				Log.Error<CharacterAnimator>("Unable to find AnimationTag with name {0} for Entity {1}!", name, Entity.Name);
				return null;
			}

			return new AnimationTag(name, id);
		}
		...
		public void SetTagValue(AnimationTag tag, bool value)
		{
			...
			{
				_animationContext.state.Set(id, value);
			}
		}
