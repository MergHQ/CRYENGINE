...
CHttpImplementationCurl::~CHttpImplementationCurl()
{
...
bool CHttpImplementationCurl::Initialize(SSystemGlobalEnvironment& env, const SSystemInitParams& initParams)
{
...
void CHttpImplementationCurl::OnPluginUpdate(EPluginUpdateType updateType)
{
	ProcessRequests();
...
void CHttpImplementationCurl::Send(IHttpPlugin::ERequestType requestType, const char* szURL, const char* szBody, TResponseCallback resultCallback, const DynArray<const char*>& headers)
{
	...
	curl_easy_setopt(request.pHandle, CURLOPT_POST, (requestType == IHttpPlugin::ERequestType::POST || requestType == IHttpPlugin::ERequestType::DELETE) ? 1 : 0);

	switch (requestType)
	{
	case IHttpPlugin::ERequestType::DELETE:
		curl_easy_setopt(request.pHandle, CURLOPT_CUSTOMREQUEST, "DELETE");
		break;
	case IHttpPlugin::ERequestType::PATCH:
		curl_easy_setopt(request.pHandle, CURLOPT_CUSTOMREQUEST, "PATCH");
		break;
	case IHttpPlugin::ERequestType::PUT:
	...
	if (headers.size() > 0)
	{
		for (const char* szHeader : headers)
		{
			request.pHeaderList = curl_slist_append(request.pHeaderList, szHeader);
		}
...
CHttpImplementationCurl::EUpdateResult CHttpImplementationCurl::ProcessRequests()
{
...
/* static */ size_t CHttpImplementationCurl::CurlWriteResponse(char *ptr, size_t size, size_t nmemb, void *userData)
{
	...
	std::shared_ptr<CHttpImplementationCurl> pHttpImplementation = CHttpImplementationCurl::CreateClassInstance();
...
/* static */ int CHttpImplementationCurl::CurlProgress(void *userData, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
{
	// Query the HTTP implementation, note that the name is unclear - nothing is created as the HTTP implementation is a singleton.
	std::shared_ptr<CHttpImplementationCurl> pHttpImplementation = CHttpImplementationCurl::CreateClassInstance();
...
CRYREGISTER_SINGLETON_CLASS(CHttpImplementationCurl)
