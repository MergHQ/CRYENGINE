...
namespace Cry
{
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			{
				SteamAPICall_t hSteamAPICall = SteamRemoteStorage()->UGCDownload(m_sharedHandle, downloadPriority);
			...
			void CSharedRemoteFile::OnDownloaded(RemoteStorageDownloadUGCResult_t* pResult, bool bIOFailure)
			{
				...
				int32 result = SteamRemoteStorage()->UGCRead(pResult->m_hFile, m_data.data(), m_data.size(), 0, k_EUGCRead_ContinueReadingUntilFinished);
				if (result == 0)
				{
