...
//
//	History:
//	- 12/07/2002: Created by Martin Mittring
//
//////////////////////////////////////////////////////////////////////
...
private:	// -----------------------------------------------------------------
	...
	public:
		...
		friend float CalcAngleBetween( const CVec3 &invA, const CVec3 &invB )
		{
			...
			if(LengthQ<0.01f)	LengthQ=0.01f;
...
//			assert(vH*vO <= 0.1f);
			...
			if(vO*ref.u>0){ vUout = vH+vO; vVout = vH-vO; }
				else { vUout = vH-vO; vVout = vH+vO; }
...
// no mesh is perfect 
	if(inU.x!=0.0f || inU.y!=0.0f || inU.z!=0.0f)assert(refBaseUV.u.x!=0.0f || refBaseUV.u.y!=0.0f || refBaseUV.u.z!=0.0f);
// no mesh is perfect
	if(inV.x!=0.0f || inV.y!=0.0f || inV.z!=0.0f)assert(refBaseUV.v.x!=0.0f || refBaseUV.v.y!=0.0f || refBaseUV.v.z!=0.0f);
