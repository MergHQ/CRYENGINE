...
#include <PRT/PRTTypes.h> // includes <windows.h>
...
//	for(yy=m_iMinY; yy<=m_iMaxY; yy++)		// juhu
	{
		...
		else
		{
			...
			{
				start=std::min(fX1toX2,fX1toX3);end=std::max(fX1toX2,fX1toX3);
			}
			else
			{
				start=std::min(fX2toX3,fX1toX3);end=std::max(fX2toX3,fX1toX3);
			}
		}
		...
		if(yy==(int)floorf(y[1]))
		{
			...
			start=std::min(start,fX1toX3);end=std::max(end,fX1toX3);
			start=std::min(start,x[1]);end=std::max(end,x[1]);
		...
		{
			start=std::min(start,x[2]);end=std::max(end,x[2]);
		}
		else
		{
			// top part without middle corner line
			if(yy<(int)floorf(y[1]))
			{
				...
				start=std::min(start,fX1toX2);end=std::max(end,fX1toX2);
			}
			else
			{
				fX2toX3+=fX2toX3step*(fSubPixelYEnd-fSubPixelYStart);
				start=std::min(start,fX2toX3);end=std::max(end,fX2toX3);
			}
