...
public:
	...
	explicit FindData(DirHeader* pRoot)
		: m_pRoot (pRoot)
		, m_pDirHeader (NULL)
	{
		m_szWildcard[0] = 0;
...
{
public:
	explicit FindFile(Cache* pCache)
		: FindData(pCache->GetRoot())
		, m_nFileEntry(0)
	{
	}
	explicit FindFile(DirHeader* pRoot)
		: FindData(pRoot)
		, m_nFileEntry(0)
	{
	}
...
{
public:
	explicit FindDir (Cache* pCache)
		: FindData(pCache->GetRoot())
		, m_nDirEntry(0)
	{
	}
	explicit FindDir(DirHeader* pRoot)
		: FindData(pRoot)
		, m_nDirEntry(0)
	{
	}
	...
	const DirEntry* GetDirEntry() const;
