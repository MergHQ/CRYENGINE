...
{
public:
	explicit FindDataRW (FileEntryTree* pRoot)
		: m_pRoot (pRoot)
		, m_pDirHeader (NULL)
	{
		m_szWildcard[0] = 0;
...
{
public:
	explicit FindFileRW (FileEntryTree* pRoot)
		: FindDataRW(pRoot)
	{
	}
...
{
public:
	explicit FindDirRW (FileEntryTree* pRoot)
		: FindDataRW(pRoot)
	{
	}
	// if bExactFile is passed, only the file is searched, and besides with the exact name as passed (no wildcards)
	...
	const FileEntryTree* GetDirEntry() const;
