...
void ImageToProcess::ConvertFromRGBA32FToR32F()
{
	...
	std::auto_ptr<ImageObject> pRet(get()->AllocateImage(0, ePixelFormat_R32F));
...
ImageObject* ImageObject::CopyAnyAlphaIntoR32FImage(int maxMipCount) const
{
	...
	std::auto_ptr<ImageObject> pRet(new ImageObject(width, height, newMipCount, ePixelFormat_R32F, GetCubemap()));
